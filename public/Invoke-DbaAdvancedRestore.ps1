                    ...
                    try {
                        ...
                        if ($OutputScriptOnly -eq $false) {
                            ...
                            [PSCustomObject]@{
                                ...
                                BackupSizeMB           = if ([bool]($backup.psobject.Properties.Name -contains 'TotalSize')) { [Math]::Round(($backup | Select-Object -ExpandProperty TotalSize | Measure-Object -Property Megabyte -Sum).Sum / $backup.FullName.Count, 2) } else { $null }
                                CompressedBackupSizeMB = if ([bool]($backup.psobject.Properties.Name -contains 'CompressedBackupSize')) { [Math]::Round(($backup | Select-Object -ExpandProperty CompressedBackupSize | Measure-Object -Property Megabyte -Sum).Sum / $backup.FullName.Count, 2) } else { $null }
                                ...
                                BackupSize             = if ([bool]($backup.psobject.Properties.Name -contains 'TotalSize')) { [dbasize](($backup | Select-Object -ExpandProperty TotalSize | Measure-Object -Property Byte -Sum).Sum / $backup.FullName.Count) } else { $null }
                                CompressedBackupSize   = if ([bool]($backup.psobject.Properties.Name -contains 'CompressedBackupSize')) { [dbasize](($backup | Select-Object -ExpandProperty CompressedBackupSize | Measure-Object -Property Byte -Sum).Sum / $backup.FullName.Count) } else { $null }
