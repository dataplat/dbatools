function Get-DbaDbTable {
    ...
    process {
        ...
        foreach ($db in $InputObject) {
            ...
            # Load all table properties in one optimized query when the config is enabled.
            # Disabled by default as it clears already loaded SMO properties, which breaks
            # scenarios where SMO objects are reused across multiple calls (e.g. Copy-DbaDbTableData).
            if (Get-DbatoolsConfigValue -FullName 'commands.get-dbadbtable.clearandinitialize') {
                # Remove properties not supported by Azure SQL
                ...
                # to avoid loading ALL tables when only specific ones are requested
                $urnFilter = ''
                if ($fqTns -or $Schema) {
