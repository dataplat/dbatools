function Install-DbaMaintenanceSolutionNew {
    <#
    .SYNOPSIS
        Installs Ola Hallengren's Maintenance Solution stored procedures and optional SQL Agent jobs for automated database maintenance (extended for SQL Server 2025 backup compression options)
    ...
    .PARAMETER Compress
 Controls whether native backup compression is forced ON, forced OFF, or left to the instance default.
 Valid values: Default, On, Off.
 - Default: does not force compression in job steps; the server/instance defaults and/or Ola defaults apply.
 - On: forces @Compress = 'Y' in all DatabaseBackup job steps.
 - Off: forces @Compress = 'N' in all DatabaseBackup job steps.
 Only applies when InstallJobs is specified.

.PARAMETER CompressionAlgorithm
 Selects the native backup compression algorithm when compression is enabled.
 SQL Server 2025 adds ZSTD as a native backup compression algorithm.
 Valid values: MS_XPRESS, QAT, ZSTD.
 Only applies when InstallJobs is specified.

.PARAMETER CompressionLevel
 Selects the native backup compression level for algorithms that support levels (e.g., ZSTD in SQL Server 2025).
 Valid values: LOW, MEDIUM, HIGH.
 Only applies when InstallJobs is specified.

.PARAMETER CompressionLevelNumeric
 Sets the numeric compression level for third-party backup tools (e.g. LiteSpeed, SQL Backup Pro, SQL Safe) when those are used via DatabaseBackup.
 If you use a third-party backup tool and you specify a compression level, you must use this numeric parameter rather than CompressionLevel.
 Only applies when InstallJobs is specified.
    ...
    .PARAMETER Verify
 Controls backup verification (RESTORE VERIFYONLY) in DatabaseBackup job steps.
 Valid values: Default, On, Off.
 - Default: leaves Ola's default behavior in place.
 - On: forces @Verify = 'Y'.
 - Off: forces @Verify = 'N'.
 Only applies when InstallJobs is specified.
    ...
    .PARAMETER CheckSum
 Controls backup checksum behavior in DatabaseBackup job steps.
 Valid values: Default, On, Off.
 - Default: leaves Ola's default behavior in place.
 - On: forces @CheckSum = 'Y'.
 - Off: forces @CheckSum = 'N'.
 Only applies when InstallJobs is specified.
    ...
    .NOTES
 Automatic downgrade mode: If -CompressionAlgorithm ZSTD or -CompressionLevel is specified on an older engine, the command downgrades automatically (ZSTD -> MS_XPRESS on 16.x; drops algorithm/level on <16) and logs a Warning.
    ...
    .LINK
        https://dbatools.io/Install-DbaMaintenanceSolution (original)
 ...
 >> CheckSum = 'On'
 >> }
 >> Install-NewDbaMaintenanceSolution @params
 Installs Ola Hallengren's solution and configures backup jobs to use native ZSTD compression with MEDIUM level (SQL Server 2025+), plus verification and checksums.

.EXAMPLE
        PS C:\> Install-NewDbaMaintenanceSolution -SqlInstance RES14224 -Database DBA -InstallJobs -CleanupTime 72
    ...
    .EXAMPLE
        PS C:\> Install-NewDbaMaintenanceSolution -SqlInstance RES14224 -Database DBA -InstallJobs -BackupLocation "Z:\SQLBackup" -CleanupTime 72
    ...
    .EXAMPLE
        ...
        >> ReplaceExisting = $true
        >> }
        PS C:\> Install-NewDbaMaintenanceSolution @params
    ...
    .EXAMPLE
        PS C:\> Install-NewDbaMaintenanceSolution -SqlInstance RES14224 -Database DBA -InstallParallel
 ...
 [ValidateSet('MS_XPRESS','QAT','ZSTD')]
 [string]$CompressionAlgorithm,
 [ValidateSet('LOW','MEDIUM','HIGH')]
 [string]$CompressionLevel,
 [int]$CompressionLevelNumeric,
        [switch]$CopyOnly,
        [ValidateSet('Default','On','Off')]
 ...
 [string]$CheckSum = 'Default',
    ...
    begin {
        ...
        if ($BackupLocation -eq "NUL" -and $Verify -ne 'Off') {
    ...
    process {
        ...
        foreach ($instance in $SqlInstance) {
            try {
                ...
                # --- SQL Server version compatibility (automatic downgrade mode) ---
        # SQL Server 2025 (VersionMajor >= 17) adds ZSTD and LOW/MEDIUM/HIGH compression levels.
        # SQL Server 2022 (VersionMajor >= 16) supports selecting MS_XPRESS or QAT algorithms.
        # Older versions do not support selecting a compression algorithm/level, so we fall back safely.
        if ($server.VersionMajor -lt 17) {
            if ($CompressionLevel) {
                Write-Message -Level Warning -Message ("CompressionLevel (LOW/MEDIUM/HIGH) requires SQL Server 2025 (VersionMajor >= 17). " +
                    "Target instance '$($server.DomainInstanceName)' reports VersionMajor $($server.VersionMajor). " +
                    "Downgrading by removing -CompressionLevel.")
                $CompressionLevel = $null
            }
            if ($CompressionAlgorithm -eq 'ZSTD') {
                if ($server.VersionMajor -ge 16) {
                    Write-Message -Level Warning -Message ("CompressionAlgorithm ZSTD requires SQL Server 2025 (VersionMajor >= 17). " +
                        "Target instance '$($server.DomainInstanceName)' reports VersionMajor $($server.VersionMajor). " +
                        "Downgrading CompressionAlgorithm to MS_XPRESS.")
                    $CompressionAlgorithm = 'MS_XPRESS'
                } else {
                    Write-Message -Level Warning -Message ("CompressionAlgorithm selection requires SQL Server 2022+ (VersionMajor >= 16). " +
                        "Target instance '$($server.DomainInstanceName)' reports VersionMajor $($server.VersionMajor). " +
                        "Downgrading by removing -CompressionAlgorithm.")
                    $CompressionAlgorithm = $null
                }
            }
        }
        if ($server.VersionMajor -lt 16 -and $CompressionAlgorithm) {
            Write-Message -Level Warning -Message ("CompressionAlgorithm selection requires SQL Server 2022+ (VersionMajor >= 16). " +
                "Target instance '$($server.DomainInstanceName)' reports VersionMajor $($server.VersionMajor). " +
                "Downgrading by removing -CompressionAlgorithm.")
            $CompressionAlgorithm = $null
            ...
            if ($InstallJobs) {
                ...
                foreach ($job in $backupJobs) {
                    ...
                    foreach ($step in $jobSteps) {
                        ...
                        # Add/normalize compression-related parameters for all backup jobs
 # Compress can be Default/On/Off. If algorithm/level is specified and Compress is Default, we implicitly turn compression On.
 $compressMode = $Compress
 if (($CompressionAlgorithm -or $CompressionLevel -or $PSBoundParameters.ContainsKey('CompressionLevelNumeric')) -and $compressMode -eq 'Default') {
     $compressMode = 'On'
 }

 # Helper to upsert a parameter assignment into the DatabaseBackup call in the job step command
 function Upsert-DbBackupParam {
     param(
         [string]$Text,
         [string]$ParamName,
         [string]$ParamValue
     )
     # If param exists, replace its assignment (handles quoted values, NULL, numbers)
     $pattern = "(?is)(@" + [regex]::Escape($ParamName) + "\s*=\s*)(N?'[^']*'|NULL|\-?\d+)"
     if ($Text -match $pattern) {
         return [regex]::Replace($Text, $pattern, ('$1' + $ParamValue))
     }

     # Otherwise, insert after @LogToTable (present in all Ola jobs)
     $anchor = "(?is)(@LogToTable\s*=\s*N?'[YN]')"
     if ($Text -match $anchor) {
         return [regex]::Replace($Text, $anchor, ('$1,' + [System.Environment]::NewLine + '@' + $ParamName + ' = ' + $ParamValue))
     }

     # Fallback: append at end
     return ($Text.TrimEnd() + ',' + [System.Environment]::NewLine + '@' + $ParamName + ' = ' + $ParamValue)
 }

 # Normalize @Compress
 switch ($compressMode) {
     'On' { $modifiedCommand = Upsert-DbBackupParam -Text $modifiedCommand -ParamName 'Compress' -ParamValue "'Y'" }
     'Off' { $modifiedCommand = Upsert-DbBackupParam -Text $modifiedCommand -ParamName 'Compress' -ParamValue "'N'" }
     default { $modifiedCommand = Upsert-DbBackupParam -Text $modifiedCommand -ParamName 'Compress' -ParamValue 'NULL' }
 }

 # Apply algorithm/level only when compression isn't Off
 if ($compressMode -ne 'Off') {
     if ($CompressionAlgorithm) {
         $modifiedCommand = Upsert-DbBackupParam -Text $modifiedCommand -ParamName 'CompressionAlgorithm' -ParamValue ("'" + $CompressionAlgorithm + "'")
     }
     if ($CompressionLevel) {
         $modifiedCommand = Upsert-DbBackupParam -Text $modifiedCommand -ParamName 'CompressionLevel' -ParamValue ("'" + $CompressionLevel + "'")
     }
     if ($PSBoundParameters.ContainsKey('CompressionLevelNumeric')) {
         $modifiedCommand = Upsert-DbBackupParam -Text $modifiedCommand -ParamName 'CompressionLevelNumeric' -ParamValue ([string]$CompressionLevelNumeric)
     }
 }
 ...
 # Add CopyOnly parameter for all backup jobs
                        ...
                        # Add/normalize Verify parameter for all backup jobs
 switch ($Verify) {
     'On' { $modifiedCommand = Upsert-DbBackupParam -Text $modifiedCommand -ParamName 'Verify' -ParamValue "'Y'" }
     'Off' { $modifiedCommand = Upsert-DbBackupParam -Text $modifiedCommand -ParamName 'Verify' -ParamValue "'N'" }
     default { }
 }

 # Add/normalize CheckSum parameter for all backup jobs
 switch ($CheckSum) {
     'On' { $modifiedCommand = Upsert-DbBackupParam -Text $modifiedCommand -ParamName 'CheckSum' -ParamValue "'Y'" }
     'Off' { $modifiedCommand = Upsert-DbBackupParam -Text $modifiedCommand -ParamName 'CheckSum' -ParamValue "'N'" }
     default { }
 }
