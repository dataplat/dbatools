...
Track the initial destination identity, the post-import staging identity, and unique staging and recovery names as the ownership boundary. Before forced promotion, verify that the current final database still has the initial identity. Rename it to recovery, verify the recovery identity, promote and identity-check staging, and only then remove recovery. If a check-to-rename race moved a replacement, restore the replacement to the final name when safe and do not promote. On promotion failure, restore the verified original when safe; otherwise preserve staging and recovery with exact recovery notes. On import or pre-promotion failure, remove staging only when its current identity matches the captured identity; preserve an unverified partial import or replacement. Never remove or modify a same-name database that appeared or was replaced after the initial destination check. In friendly mode emit `Failed` and continue; with `EnableException`, call `Stop-Function` with a sanitized error record and stop. In `finally`, remove the BACPAC with `Remove-Item -LiteralPath` unless `KeepBacpac` is set. Record stopwatch elapsed time for every emitted status.
