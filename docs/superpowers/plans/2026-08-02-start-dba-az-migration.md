...
10. Replaces an existing destination database only with `Force`, after a complete staging import is ready.
...
- Validate in the public command scope that `Path` is an existing directory or can be created, then return immediately on friendly-mode failure.
...
Check the destination by exact database name. Without `Force`, emit `Skipped` with `Already exists on destination`. With `Force`, use one `ShouldProcess` decision for the combined staged replacement migration. Under `WhatIf`, create no package and perform no destination mutation. Export and import the replacement into a unique staging database before dropping the existing destination, so export/import failures leave that database intact.
...
Validate that exactly one returned object has an existing `Path`, record that path, generate an unguessable staging database name, and import with:
...
Require one publish result and verify the staging database exists. Capture its identity, then recheck the requested final name. If it appeared after the initial check, leave it untouched and fail safely. For an approved forced replacement, verify the original identity, rename it to the unique recovery name, verify that recovery still has the original identity, promote and verify staging, and only then remove recovery. Mark the status `Successful` after that sequence completes.

- [ ] **Step 8: Implement failure isolation and cleanup**

Track the initial destination identity, the post-import staging identity, and GUID-based staging and recovery names as the ownership boundary. Read identity from Azure SQL's `sys.databases.service_broker_guid` and logical-server-unique `database_id`; live Azure validation confirms that both values follow a database across rename and distinguish a same-name replacement. Before forced promotion, verify that the current final database still has the initial identity. Rename it to recovery, verify the recovery identity, promote and identity-check staging, and only then remove recovery. If a check-to-rename race moved a replacement, restore the replacement to the final name when safe and do not promote. On promotion failure, restore the verified original when safe; otherwise preserve staging and recovery with exact recovery notes. On import or pre-promotion failure, remove staging only when its current identity matches the captured identity; preserve an unverified partial import or replacement. Reread identity immediately before every name-based deletion. Azure SQL provides no atomic identity-conditional delete, so a privileged actor intentionally enumerating and hijacking a run-owned GUID name is outside the threat model. Never remove or modify a same-name database that appeared or was replaced after the initial destination check. In friendly mode emit `Failed` and continue; with `EnableException`, call `Stop-Function` with a sanitized error record and stop. In `finally`, remove the BACPAC with `Remove-Item -LiteralPath` unless `KeepBacpac` is set. Record stopwatch elapsed time for every emitted status.
