function Set-DbaDbCompression {
    <#
        ...
        .EXAMPLE
            ...
            Set the compression run time to 60 minutes and will start the compression of tables/indexes within the specified database. Only objects that have a difference of 25% or higher between current and recommended will be compressed and the results into a nicely formated GridView.
        ...
        foreach ($instance in $SqlInstance) {
            ...
            foreach ($db in $dbs) {
                ...
                try {
                    ...
                    foreach ($obj in ($results | Where-Object {$_.CompressionTypeRecommendation -ne 'NO_GAIN' -and $_.PercentCompression -ge $PercentCompression} | Sort-Object PercentCompression -Descending)) {                        #check time limit isn't met
                        if (($(get-date) - $starttime).Minutes -ge $MaxRunTime) {
