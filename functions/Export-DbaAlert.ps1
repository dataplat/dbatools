Function Export-DbaAlert {
    <#
	.SYNOPSIS
	Export one, many or all SQL Server Agent alerts

	.DESCRIPTION
	Exports one, many or all SQL Server Agent alerts as T-SQL output
	...
	.PARAMETER Alerts
	By default, all alerts are exported. This parameters allows you to export only specific alerts
	...
	.LINK
	https://dbatools.io/Export-DbaAlert
	...
	Exports alerts and replaces all instances of the servername "sql2014" with "sql2016" then writes to C:\temp\export.sql
	#>
	...
	dynamicparam {
		if ($SqlInstance) {
			return Get-ParamSqlAlerts -SqlServer $SqlInstance[0] -SqlCredential $SqlCredential
		}
	}
...
*/"
			...
			foreach ($alert in $exportalerts) {
				If ($Pscmdlet.ShouldProcess($env:computername, "Exporting $alert from $server to $actualpath")) {
					Write-Message -Level Verbose -Message "Exporting $alert"
					
					if ($passthru) {
						$alert.Script() | Out-String
					}
					else {
						$alert.Script() | Out-File -FilePath $actualpath -Encoding $encoding -Append
					}
				}
			}
