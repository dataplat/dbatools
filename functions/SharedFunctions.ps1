...
#>	
	...
	[scriptblock]$setupScriptBlock = {
		$Server = $args[$args.GetUpperBound(0)]
	...
	catch
	{
		try
		{
			if ($credential.username -ne $null)
			{
				Invoke-Command -ScriptBlock $ScriptBlock -ArgumentList $ArgumentList -Credential $Credential -ComputerName [System.Net.Dns]::gethostentry($ipaddr)
			}
			else
			{
				Invoke-Command -ScriptBlock $ScriptBlock -ArgumentList $ArgumentList -ComputerName [System.Net.Dns]::gethostentry($ipaddr)
