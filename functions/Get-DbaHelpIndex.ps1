...
#>
	...
	BEGIN
	{
		...
		IF (!$ObjectName)
		{
			$TablePredicate = " @TableName NVARCHAR(256)";
		}
		ELSE
		{
			$TablePredicate = " @TableName NVARCHAR(256) = '$ObjectName'";
		}
		...
		IF ($IncludeDataTypes)
		{
			$IncludeDataTypesPredicate = '@IncludeDataTypes BIT = 1';
		}
		ELSE
		{
			$IncludeDataTypesPredicate = '@IncludeDataTypes BIT = 0';
		}
		...
		if ($databases.Count -gt 0)
		{
			foreach ($db in $databases)
			{
				try
				{
					...
					$IndexDetails = ($server.Databases[$db].ExecuteWithResults($SizesQuery)).Tables[0];
