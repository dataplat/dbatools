function Set-DbaAgentOperator {
    <#
    .SYNOPSIS
        Changes settings for an operator on an instance.

    .DESCRIPTION
        Changes settings for an operator.
    ...
    .EXAMPLE
        PS:\> Set-DbaAgentOperator -SqlInstance sql01 -Operator DBA -OperatorEmail operator@operator.com -PagerDay Everyday

        Changes the oprator named DBA with the above email address with default values to alerts everyday
        for all hours of the day.

        PS:\> Set-DbaAgentOperator -SqlInstance sql01 -Operator DBA -IsFailsafeOperator

        Changes the operator to the failsafe operator.
    ...
    process {
        foreach ($instance in $SqlInstance) {
            ...
            if ((Get-DbaAgentOperator -SqlInstance $instance -Operator $Operator).Count -ne 0) {
                Stop-Function -Message "Operator $Operator does not exist on $instance." -Target $instance
                return
            } else {
                $operator = $server.JobServer.Operators | Where-Object { $Operator -contains $_.Name }
            }
            ...
            if ($NetSendAddress) {
                $operator.NetAddress = $NetAddress
            }
            ...
            if ($PagerDay -and $PagerDay -notin ('EveryDay', 'Weekdays', 'Weekend', 'Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday')) {
                Stop-Function -Message "PagerDay must contain one of these values 'EveryDay', 'Weekdays', 'Weekend', 'Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'."
                return
            } else {
                [int]$Interval = 0
                ...
                if (-not $SaturdayStartTime -and $Force) {
                    ...
                    return
                } elseif ($SaturdayStartTime -notmatch $RegexTime -and $PagerDay -in ('Everyday', 'Saturday', 'Weekends')) {
                ...
                if (-not $SaturdayEndTime -and $Force) {
                    ...
                    return
                } elseif ($SaturdayEndTime -notmatch $RegexTime -and $PagerDay -in ('Everyday', 'Saturday', 'Weekends')) {
                ...
                if (-not $SundayStartTime -and $Force) {
                    ...
                    return
                } elseif ($SundayStartTime -notmatch $RegexTime -and $PagerDay -in ('Everyday', 'Sunday', 'Weekends')) {
                ...
                if (-not $SundayEndTime -and $Force) {
                    ...
                    return
                } elseif ($SundayEndTime -notmatch $RegexTime -and $PagerDay -in ('Everyday', 'Sunday', 'Weekends')) {
                ...
                if (-not $WeekdayStartTime -and $Force) {
                    ...
                    return
                } elseif ($WeekdayStartTime -notmatch $RegexTime -and $PagerDay -in ('Everyday', 'Weekdays', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday')) {
                ...
                if (-not $WeekdayEndTime -and $Force) {
                    ...
                    return
                } elseif ($WeekdayEndTime -notmatch $RegexTime -and $PagerDay -in ('Everyday', 'Weekdays', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday')) {
                ...
                if ($SaturdayStartTime -and $PagerDay -in ('Everyday', 'Saturday', 'Weekends')) {
                    $operator.SaturdayPagerStartTime = $SaturdayStartTime
                }
                if ($SaturdayEndTime -and $PagerDay -in ('Everyday', 'Saturday', 'Weekends')) {
                    $operator.SaturdayPagerEndTime = $SaturdayEndTime
                }

                if ($SundayStartTime -and $PagerDay -in ('Everyday', 'Sunday', 'Weekends')) {
                    $operator.SundayPagerStartTime = $SundayStartTime
                }
                if ($SundayEndTime -and $PagerDay -in ('Everyday', 'Sunday', 'Weekends')) {
                    $operator.SundayPagerEndTime = $SundayEndTime
                }

                if ($WeekdayStartTime -and $PagerDay -in ('Everyday', 'Weekdays', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday')) {
                    $operator.WeekdayPagerStartTime = $WeekdayStartTime
                }
                if ($WeekdayEndTime -and $PagerDay -in ('Everyday', 'Weekdays', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday')) {
            ...
            if ($Pscmdlet.ShouldProcess($instance, "Alteringting Operator $operator")) {
                try {
                    $server.JobServer.Operators[$Operator].Alter()

                    Write-Message -Level Verbose -Message "Altering Operator $operator"
                    Get-DbaAgentOperator -SqlInstance $instance -Operator $Operator
                } catch {
                    Stop-Function -Message "Issue alter operator." -Category InvalidOperation -ErrorRecord $_ -Target $instance
                }
            }
        }
    }
