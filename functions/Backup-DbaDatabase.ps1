                    ...
                    if ($Pscmdlet.ShouldProcess($server.Name, "Backing up $dbname to $humanBackupFile")) {
                        ...
                        if ($OutputScriptOnly -ne $True) {
                            ...
                            if ($server.VersionMajor -eq '8') {
                                ...
                                $HeaderInfo = Get-DbaBackupHistory -SqlInstance $server -Database $dbname -Last -IncludeCopyOnly:$CopyOnly -LastLsn $NumericLsn  | Sort-Object -Property End -Descending | Select-Object -First 1
                            }
                            $Verified = $false
                            if ($Verify) {
