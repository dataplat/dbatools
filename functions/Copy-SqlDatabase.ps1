...
#>	
		...
		{			
				...
				foreach ($database in $databaselist)
				{
					...
					if ($BackupRestore)
					{
						...
						{
							$result = (Start-SqlBackupRestore $sourceserver $destserver $dbname $networkshare)
							$dbfinish = Get-Date
							if ($result -eq $true)
							{
	...
	END
	{
		If ($Pscmdlet.ShouldProcess("console", "Showing migration time elapsed"))
		{
			...
			if ($networkshare.length -gt 0 -and $NoBackupCleanup) { Write-Warning "Backups still exist at $networkshare." }
		}
	}
}