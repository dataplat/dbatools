	...
	PROCESS
	{
		...
		foreach ($server in $servers)
		{
            ...
            else
            {
                ...
                {
                    Write-Warning "Server '$servername' (v$($server.versionMajor)) does not support Max DOP configuration at database level. Remember that this option is only available from SQL Server 2016 (v13). Run the command again without using database related parameters. Skipping."
				    Continue
			    }
            }
