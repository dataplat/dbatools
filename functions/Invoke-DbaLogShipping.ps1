function Invoke-DbaLogShipping {
    ...
    begin {
        ...
        # Try connecting to the instance
        Write-Message -Message "Connecting to destination Sql Server $DestinationSqlInstance.." -Level Verbose
        try {
    ...
    process {
        ...
        foreach ($db in $DatabaseCollection) {
            ...
            [PSCustomObject]@{
                PrimaryInstance = $SourceServer.InstanceName
                SecondaryInstance = $DestinationServer.InstanceName
                PrimaryDatabase = $($db.Name)
                SecondaryDatabase = $SecondaryDatabase
                Result = $setupResult
                Comment = $comment
