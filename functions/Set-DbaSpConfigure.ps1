			...
			If ($Pscmdlet.ShouldProcess($SqlInstance, "Adjusting server configuration $Config from $currentRunValue to $value.")) {
				try {
					...
					if ($isDynamic -eq $false) {
						Write-Message -Level Warning -Message "Configuration item $ConfigName has been set, but restart of SQL Server is required for the new value $value to be used (old value: $currentRunValue)" -Target $Instance
					}
				}
				catch {
