function Reset-DbaAdmin {
    ...
    begin {
        ...
        function Invoke-ResetSqlCmd {
            ...
            try {
                $connstring = "Data Source=$instance;Integrated Security=True;Connect Timeout=2;Application Name=Reset-DbaAdmin"
        ...
        if ($SqlCredential) {
            ...
            $SecurePassword = $SqlCredential.SecurePas
        }
    }
    ...
    process {
        foreach ($instance in $sqlinstance) {
            ...
            if ($pscmdlet.ShouldProcess($baseaddress, "Reset-DbaAdmin (SQL Server instance $instance will restart)")) {
                # Get hostname
                ...
                if ($baseaddress -ne $env:COMPUTERNAME) {
                    ...
                    if ($testconnect -eq $false) {
                        ...
                        try {
                            ...
                            Stop-Function -Continue -Message "Can't connect to $baseaddress either via ping or tcp (WMI port 135)"
                        }
                    }
                    ...
                    try {
                        ...
                        Stop-Function -Continue -Message "Could not resolve SqlServer IP or NetBIOS name"
                    }
                    
                    Write-Message -Level Verbose -Message "Resolving NetBIOS name."
                    try {
                        $hostname = (Get-DbaCmObject -ClassName Win32_NetworkAdapterConfiguration -ComputerName $ipaddr | Where-Object IPEnabled | Select-Object -First 1).PSComputerName
                        if ($null -eq $hostname) {
                        ...
                        Stop-Function -Continue -Message "Could not access remote WMI object. Check permissions and firewall."
                    }
                }
                ...
                if (-not $instance.IsLocalHost) {
                    try {
                        ...
                        Stop-Function -Continue -Message "Can't access $hostname using PSSession. Check your firewall settings and ensure Remoting is enabled or run the script locally."
                    }
                }
                ...
                try {
                    $null = Invoke-ResetSqlCmd -SqlInstance $instance -Sql "SELECT 1" -ErrorAction Stop
                } catch {
                    try {
                        Start-Sleep 3
                        $null = Invoke-ResetSqlCmd -SqlInstance $instance -Sql "SELECT 1" -ErrorAction Stop
                ...
                if ($windowslogin -eq $true) {
                    ...
                    if ($(Invoke-ResetSqlCmd -SqlInstance $instance -Sql $sql) -eq $false) {
