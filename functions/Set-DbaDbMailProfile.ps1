function Set-DbaDbMailProfile {
    ...
    process {
        if ($SqlInstance) {
            ...
            $null = $params.Remove('MailAccountName')
            $null = $params.Remove('MailAccountPriority')
...
#                    if (Test-Bound -ParameterName 'MailAccountName') {
                        ...
                        $dbMailProfile.AddAccount($MailAccountName, $MailAccountPriority) # sequenceNumber correlates to "Priority" when associating a db mail Account to a db mail Profile
                        $MailAccountName
                        $MailAccountPriority
#                    }
