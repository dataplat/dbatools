...
`           ##################################################################################
            ...
            try {
                ...
                Stop-Function -Message "Error occured while establishing connection to $instance" -Category ConnectionError -ErrorRecord $_ -Target $DistributionServer -Continue
            }
