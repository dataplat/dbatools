...
# Runspaces are flaky, ensure these work
...
if ($script:serialimport) {
    ...
    $script:teppRunspace = [System.Management.Automation.PowerShell]::Create()
    if ($script:teppRunspace.Runspace.Name) {
        try { $script:teppRunspace.Runspace.Name = "dbatools-import-tepp" }
        catch { }
    }
    $script:teppRunspace.AddScript($scriptBlock)
    $script:teppRunspace.BeginInvoke()
}