...
Set-DbatoolsConfig -FullName "TabExpansion.Asynchronous.StartOnImport" -Value $false -Initialize -Validation bool -Description "Starts the asynchronous TEPP cache runspace during module import, the way dbatools did before the first tab completion became the trigger. Off by default, because a session that never tab completes anything has no use for the cache and the runspace reconnects to every instance the session has touched."