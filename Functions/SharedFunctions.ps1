...
Function Invoke-SMOCheck {
	...
	if ($script:smocheck -ne $true) {
		...
		if ($serverversion-$smo -gt 1) {
			Write-Warning "Your version of SMO is $smo, which is significantly older than the $($sqlserver.name) version $($SqlServer.version.major)."
...
Function Test-SqlConnection  {
	<# 
	 .SYNOPSIS 
	 Tests a the connection to a single instance.
	 
	  .EXAMPLE
     Test-SqlConnection -Server sqlserver -SqlCredential $SqlCredential
