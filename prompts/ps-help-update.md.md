...
5. **Research SMO and .NET objects**:
   - When the output is a Microsoft.SqlServer.Management.Smo.* object, search learn.microsoft.com for the object documentation
   - Verify which properties actually exist on the SMO object before listing them
   - Only list properties that are documented or that you can confirm exist
   - Use the Microsoft documentation as the authoritative source for SMO object properties
   - If you cannot find documentation or verify a property exists, do not include it in the "Additional properties" section

6. **Update the file**:
   - Locate the existing `.OUTPUTS` section in the comment-based help (within the `<# ... #>` block)
   - Replace the entire `.OUTPUTS` section with your new documentation
   - If no `.OUTPUTS` section exists, add it after `.PARAMETER` sections and before `.EXAMPLE`
   - Preserve all other parts of the comment-based help exactly as they are
   - Maintain the original indentation style of the file

## Output Format

Generate a `.OUTPUTS` section in PowerShell comment-based help format, then update the file with this section.
...
- **Research SMO objects** - verify properties on learn.microsoft.com before documenting them as available
- **Maintain indentation** - match the existing indentation style in the file (typically 8 spaces for `.OUTPUTS` and 12 spaces for content)

## Final Step

After analyzing the code and generating the `.OUTPUTS` documentation:
1. Locate the existing `.OUTPUTS` section (or determine where to insert it)
2. Replace or insert the new `.OUTPUTS` section
