...
'@
    ...
    It -Skip:(-not $env:azurepasswd) "sets up log shipping to Azure blob storage using SAS token" {
        ...
        $primaryServer = Connect-DbaInstance -SqlInstance localhost
        ...
        $secondaryServer = Connect-DbaInstance -SqlInstance localhost:14333
        if (Get-DbaCredential -SqlInstance localhost:14333 -Name "[$azureUrl]") {
            $secondaryServer.Query("DROP CREDENTIAL [$azureUrl]")
        }
        $secondaryServer.Query($sql)

        # Create test database
        $null = New-DbaDatabase -SqlInstance localhost -Name $dbName
        ...
        $jobs = Get-DbaAgentJob -SqlInstance localhost:14333
        ...
        # Cleanup
        $null = Remove-DbaDbLogShipping -SqlInstance localhost -Database $dbName -WarningAction SilentlyContinue
        $null = Remove-DbaDbLogShipping -SqlInstance localhost:14333 -Database $dbName -WarningAction SilentlyContinue
        $null = Remove-DbaDatabase -SqlInstance localhost -Database $dbName
    ...
    It -Skip:(-not $env:azurelegacypasswd) "sets up log shipping to Azure blob storage using storage account key" {
        ...
        $primaryServer = Connect-DbaInstance -SqlInstance localhost
        if (Get-DbaCredential -SqlInstance localhost -Name $credName) {
        ...
        $secondaryServer = Connect-DbaInstance -SqlInstance localhost:14333
        if (Get-DbaCredential -SqlInstance localhost:14333 -Name $credName) {
