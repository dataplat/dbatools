		...
		function Get-SQLInstanceDetail {
			...
			process {
				...
				foreach ($sqlInstance in $Instance) {
					...
					Write-Message -Level Debug -Message "Working with $regPath\$instanceValue on $computer"
					...
					try {
						...
						if ($patchVersion = $instanceRegSetup.GetValue("PatchLevel")) {
							Write-Message -Level Debug -Message "Using patch version $patchVersion over $version"
						...
						Write-Message -Level Debug -Message "Failed to read one of the reg keys, found version $version so far"
					}
