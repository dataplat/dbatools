function Invoke-DbaCmd
{
    ...
    .DESCRIPTION
        Runs a T-SQL script. Invoke-DbaCmd runs the whole scipt and only captures the first selected result set, such as the output of PRINT statements when -verbose parameter is specified.
    ...
    .PARAMETER Database
        A character string specifying the name of a database. Invoke-DbaCmd connects to this database in the instance that is specified in -ServerInstance.
    ...
    .PARAMETER InputFile
        Specifies a file to be used as the query input to Invoke-DbaCmd. The file can contain Transact-SQL statements, (? XQuery statements, and sqlcmd commands and scripting variables ?). Specify the full path to the file.
    ...
    .PARAMETER ConnectionTimeout
        Specifies the number of seconds when Invoke-DbaCmd times out if it cannot successfully connect to an instance of the Database Engine. The timeout value must be an integer between 0 and 65534. If 0 is specified, connection attempts do not time out.
    ...
    .INPUTS
        None
            You cannot pipe objects to Invoke-DbaCmd
    ...
    .EXAMPLE
        Invoke-DbaCmd -ServerInstance "MyComputer\MyInstance" -Query "SELECT login_time AS 'StartTime' FROM sysprocesses WHERE spid = 1"
    ...
    .EXAMPLE
        Invoke-DbaCmd -ServerInstance "MyComputer\MyInstance" -InputFile "C:\MyFolder\tsqlscript.sql" | Out-File -filePath "C:\MyFolder\tsqlscript.rpt"

        This example reads a file containing T-SQL statements, runs the file, and writes the output to another file.

    .EXAMPLE
        Invoke-DbaCmd  -ServerInstance "MyComputer\MyInstance" -Query "PRINT 'hello world'" -Verbose

        This example uses the PowerShell -Verbose parameter to return the message output of the PRINT command.
        VERBOSE: hello world

    .EXAMPLE
        Invoke-DbaCmd -ServerInstance MyServer\MyInstance -Query "SELECT ServerName, VCNumCPU FROM tblServerInfo" -as PSObject | ?{$_.VCNumCPU -gt 8}
        Invoke-DbaCmd -ServerInstance MyServer\MyInstance -Query "SELECT ServerName, VCNumCPU FROM tblServerInfo" -as PSObject | ?{$_.VCNumCPU}
    ...
    .EXAMPLE
        'Instance1', 'Server1/Instance1', 'Server2' | Invoke-DbaCmd -query "Sp_databases" -as psobject -AppendServerInstance
    ...
    .EXAMPLE
        ...
        #Run the query, specifying values for SQL parameters
            Invoke-DbaCmd -ServerInstance SomeServer\NamedInstance -Database ServerDB -query $query -SqlParameters @{ VCServerContact="%cookiemonster%"; VCServerClass="Prod" }
    ...
    .EXAMPLE
        Invoke-DbaCmd -SQLConnection $Conn -Query "SELECT login_time AS 'StartTime' FROM sysprocesses WHERE spid = 1"
    ...
    .LINK
        https://github.com/sqlcollaborative/Invoke-DbaCmd
	...
	Begin
	{
		...
		Write-Verbose "Running Invoke-DbaCmd with ParameterSet '$($PSCmdlet.ParameterSetName)'.  Performing query '$Query'"
...
'@
	...
	END { Test-DbaDeprecation -DeprecatedOn "1.0.0" -Alais Invoke-DbaCmd }
} #Invoke-DbaCmd