...
$assemblyPath = Join-Path -Path $PSScriptRoot -ChildPath "$editionFolder\dbatools.ssis.dll"
if (-not (Test-Path -Path $assemblyPath)) {
    # Dev-staging fallback: the flip preflight also stages a flat copy at the module root
    $assemblyPath = Join-Path -Path $PSScriptRoot -ChildPath 'dbatools.ssis.dll'
}
