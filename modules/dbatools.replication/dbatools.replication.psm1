...
# SQL Server assemblies here (that includes the RMO assemblies the cmdlet dll leans on).
...
$assemblyPath = Join-Path -Path $PSScriptRoot -ChildPath "$editionFolder\dbatools.replication.dll"
if (-not (Test-Path -Path $assemblyPath)) {
    # Dev-staging fallback: the flip preflight also stages a flat copy at the module root
    $assemblyPath = Join-Path -Path $PSScriptRoot -ChildPath 'dbatools.replication.dll'
}
