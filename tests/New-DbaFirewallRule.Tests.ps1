...
Describe "New-DbaFirewallRule" {
    ...
    Context "Integration Tests" {
        BeforeAll {
            ...
            $null = Remove-DbaFirewallRule -SqlInstance $env:instance2 -Confirm:$false
        }
        ...
        It "creates two firewall rules" {
            $resultsNew = New-DbaFirewallRule -SqlInstance $env:instance2 -Confirm:$false
            $resultsNew.Count | Should -Be 2
        }

        It "creates first firewall rule for SQL Server instance" {
            $resultsNew = New-DbaFirewallRule -SqlInstance $env:instance2 -Confirm:$false
            $instanceName = ([DbaInstanceParameter]$env:instance2).InstanceName
        ...
        It "returns two firewall rules" {
            $resultsGet = Get-DbaFirewallRule -SqlInstance $env:instance2
        ...
        It "removes other firewall rule" {
            $resultsRemove = Remove-DbaFirewallRule -SqlInstance $env:instance2 -Type AllInstance -Confirm:$false
