...
SELECT DatabaseName, AssemblyName FROM @inventory;"
        ...
        $null = New-DbaDatabase -SqlInstance $sourceInstance -Name $fixtureDb1, $fixtureDb2
    ...
    AfterAll {
        ...
        # Anything an unfiltered leg pushed into a destination database that is not a fixture
        # database. Compared against the pre-run inventory so an assembly that was already there is
        # never dropped.
        try {
            $strayAssemblies = @(Invoke-DbaQuery -SqlInstance $destInstance -Query $assemblyInventoryQuery) |
            ...
            foreach ($strayAssembly in $strayAssemblies) {
                $dropStrayParameters = @{
                    assemblyName = $strayAssembly.AssemblyName
                }
                $splatDropStrayAssembly = @{
                    SqlInstance  = $destInstance
                    Database     = $strayAssembly.DatabaseName
                    Query        = "DECLARE @statement nvarchar(max); SET @statement = N'DROP ASSEMBLY ' + QUOTENAME(@assemblyName); EXEC sp_executesql @statement;"
                    SqlParameter = $dropStrayParameters
                }
                Invoke-DbaQuery @splatDropStrayAssembly
            }
        } catch {
            $cleanupFailures += "stray destination assemblies: $($PSItem.Exception.Message)"
        }

        # Copying an external-access assembly turns the destination database TRUSTWORTHY on, and
        # that database is not always one of ours - it is any database whose name matches a source
        # database. Put back only the flags this run changed.
        try {
            ...
            foreach ($databaseState in $currentTrustworthy) {
                ...
                $restoreTrustworthyParameters = @{
                    databaseName = $databaseState.DatabaseName
                }
                $trustworthySetting = if ($originalState[0].IsTrustworthy -eq 1) { "ON" } else { "OFF" }
                $splatRestoreTrustworthy = @{
                    SqlInstance  = $destInstance
                    Query        = "DECLARE @statement nvarchar(max); SET @statement = N'ALTER DATABASE ' + QUOTENAME(@databaseName) + N' SET TRUSTWORTHY $trustworthySetting'; EXEC sp_executesql @statement;"
                    SqlParameter = $restoreTrustworthyParameters
                }
                Invoke-DbaQuery @splatRestoreTrustworthy
            }
        } catch {
            $cleanupFailures += "destination TRUSTWORTHY flags: $($PSItem.Exception.Message)"
        }
    ...
    Context "When copying database assemblies" {
        ...
        It "Should copy the assembly and report one status object per source database" {
            ...
            # narrowed to the fixture; the count assertion below is what proves nothing else on the
            # source carried an assembly into this run, and AfterAll removes anything that did.
        ...
        It "Should honor -ExcludeAssembly when no -Assembly list is supplied" {
            ...
            # branch than the one the title claims. It creates nothing on the destination that the
            # leg above has not already created, so it adds no side effect of its own.
