...
Describe "Set-DbaAgentAlert" {
    ...
    Context "Command usage" {
        BeforeAll {
            $server = Connect-DbaInstance -SqlInstance $env:instance2 -Database master
        ...
        It "changes new alert to disabled" {
            $results = Set-DbaAgentAlert -SqlInstance $env:instance2 -Alert 'dbatoolsci test alert' -Disabled
            $results.IsEnabled | Should -Be 'False'
        }

        It "changes new alert name to dbatoolsci test alert NEW" {
            $results = Set-DbaAgentAlert -SqlInstance $env:instance2 -Alert 'dbatoolsci test alert' -NewName 'dbatoolsci test alert NEW'
