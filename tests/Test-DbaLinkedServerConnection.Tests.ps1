...
Describe $CommandName -Tag IntegrationTests {
    ...
    Context "Function works" {
        BeforeAll {
            $results = Test-DbaLinkedServerConnection -SqlInstance $TestConfig.InstanceRestart | Where-Object LinkedServerName -eq $target
        }
    ...
    Context "Piping to function works" {
        BeforeAll {
            $pipeResults = Get-DbaLinkedServer -SqlInstance $TestConfig.InstanceRestart | Test-DbaLinkedServerConnection
        }
