...
Describe "$CommandName Integration Tests" -Tag "IntegrationTests" {
    ...
    Context "Database is properly removed (name change)" {
        ...
        Context "Test restoring as SA #6992" {
            ...
            $results = Get-ChildItem $script:appveyorlabrepo\singlerestore\singlerestore.bak | Restore-DbaDatabase -SqlInstance $script:instance2 -DatabaseName Pestering -replaceDbNameInFile -WithReplace -Execute badlogin -WarningVariable warnvar
            $db = Get-DbaDatabase -SqlInstance $script:instance2 -Database Pestering
            It "Should throw a warning if login doesn't exist" {
                $warnvar | Should -Be "You specified a Login to execute the restore, but the login 'badlogin' does not exist"
            }
            ...
            New-DbaLogin -SqlInstance $script:instance2 -Login $RestoreAsUser -SecurePassword (ConvertTo-SecureString 'P@ssw0rdl!ng' -AsPlainText -Force)
            Add-DbaServerRoleMember -SqlInstance server1 -Role sysadmin -Login $RestoreAsUser
            ...
            $db = Get-DbaDatabase -SqlInstance $script:instance2 -Database Pestering
            It "Should be owned by $RestoreAs this time" {
                $db.owner | Should -Be $RestoreAs
            }
        ...
        Context "Continue Restore with multiple databases" {
            ...
            $Results2 = $files | Where-Object { $_.PsIsContainer -eq $false } | Restore-DbaDatabase -SqlInstance $script:instance2 -Continue
            It "Should Have restored the database cleanly" {
                ($results2.RestoreComplete -contains $false) | Should be $False
                (($results2 | Measure-Object).count -gt 0) | Should be $True
            }
