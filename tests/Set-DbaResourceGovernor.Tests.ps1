...
Describe "Set-DbaResourceGovernor" {
    ...
    Context "Command actually works" {
        ...
        BeforeAll {
            ...
            RETURN DB_NAME();
            END;"
            Invoke-DbaQuery -SqlInstance $env:instance2 -Query $createUDFQuery -Database "master"
            Set-DbaResourceGovernor -SqlInstance $env:instance2 -Disabled -Confirm:$false
        }

        It "enables resource governor" {
            $results = Set-DbaResourceGovernor -SqlInstance $env:instance2 -Enabled -Confirm:$false
            $results.Enabled | Should -Be $true
        }

        It "disables resource governor" {
            $results = Set-DbaResourceGovernor -SqlInstance $env:instance2 -Disabled -Confirm:$false
        ...
        It "modifies resource governor classifier function" {
            ...
            $results = Set-DbaResourceGovernor -SqlInstance $env:instance2 -ClassifierFunction $classifierFunction -Confirm:$false
            $results.ClassifierFunction | Should -Be $qualifiedClassifierFunction
        }

        It "removes resource governor classifier function" {
            $results = Set-DbaResourceGovernor -SqlInstance $env:instance2 -ClassifierFunction 'NULL' -Confirm:$false
            $results.ClassifierFunction | Should -Be ''
        }

        AfterAll {
            $dropUDFQuery = "DROP FUNCTION [dbo].[$classifierFunction];"
            Invoke-DbaQuery -SqlInstance $env:instance2 -Query $dropUDFQuery -Database "master"
        }
    }
}
