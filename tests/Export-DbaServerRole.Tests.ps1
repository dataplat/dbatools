...
Describe "$commandname Integration Tests" -Tags "IntegrationTests" {
    ...
    Context "Check using piped input created" {
        ...
        It "should include the role" {
            $global:results -match "CREATE SERVER ROLE [$svRole]"
        }
        It "should include ADD MEMBER" {
            $global:results -match "ALTER SERVER ROLE [dbcreator] ADD MEMBER [$svRole]"
        }
        It "should include GRANT CREATE TRACE EVENT" {
            $global:results -match "GRANT CREATE TRACE EVENT NOTIFICATION TO [$svRole]"
        }
        It "should include DENY SELECT ALL USER SECURABLES" {
            $global:results -match "DENY SELECT ALL USER SECURABLES TO [$svRole]"
        }
        It "should include VIEW ANY DEFINITION" {
            $global:results -match "GRANT VIEW ANY DEFINITION TO [$svRole];"
        }
        It "should include GRANT VIEW ANY DATABASE" {
            $global:results -match "GRANT VIEW ANY DATABASE TO [$svRole];"
        }
    }
}