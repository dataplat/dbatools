...
Describe "$commandname Integration Tests" -Tag "IntegrationTests" {
    ...
    Context "Parameters validation" {
        It "Stops if no Database or AllDatabases" {
            { Remove-DbaDbSnapshot -SqlInstance $global:instance2 -EnableException -WarningAction SilentlyContinue } | Should Throw "You must pipe"
        }
        It "Is nice by default" {
            { Remove-DbaDbSnapshot -SqlInstance $global:instance2 *> $null } | Should Not Throw "You must pipe"
        }
    }
