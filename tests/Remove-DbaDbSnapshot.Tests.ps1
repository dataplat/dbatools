...
Describe "$commandname Integration Tests" -Tags "IntegrationTests" {
    ...
    Context "Parameters validation" {
        It "Stops if no Database or AllDatabases" {
            { Remove-DbaDbSnapshot -SqlInstance $script:instance2 -EnableException } | Should Throw "You must specify"
        }
        It "Is nice by default" {
            { Remove-DbaDbSnapshot -SqlInstance $script:instance2 *> $null } | Should Not Throw "You must specify"
        }
    }
    ...
    Context "Operations on snapshots" {
        BeforeEach {
            $needed = @()
            $needed += New-DbaDbSnapshot -SqlInstance $script:instance2 -Database $db1 -Name $db1_snap1 -ErrorAction SilentlyContinue
            $needed += New-DbaDbSnapshot -SqlInstance $script:instance2 -Database $db1 -Name $db1_snap2 -ErrorAction SilentlyContinue
            $needed += New-DbaDbSnapshot -SqlInstance $script:instance2 -Database $db2 -Name $db2_snap1 -ErrorAction SilentlyContinue
        ...
        if ($setupright) {
            It "Honors the Database parameter, dropping only snapshots of that database" {
                $results = Remove-DbaDbSnapshot -SqlInstance $script:instance2 -Database $db1 -Force
                $results.Count | Should Be 2
                $result = Remove-DbaDbSnapshot -SqlInstance $script:instance2 -Database $db2 -Force
            ...
            It "Honors the ExcludeDatabase parameter, returning relevant snapshots" {
                ...
                $results = Remove-DbaDbSnapshot -SqlInstance $script:instance2 -ExcludeDatabase $alldbs -Force
            ...
            It "Works with piped snapshots" {
                $result = Get-DbaDbSnapshot -SqlInstance $script:instance2 -Snapshot $db1_snap1 | Remove-DbaDbSnapshot -Force
