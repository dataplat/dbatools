...
Describe $CommandName -Tag IntegrationTests {
    BeforeAll {
        $results = Get-DbaWsfcNetworkInterface -ComputerName $TestConfig.InstanceHadr -WarningVariable WarnVar -OutVariable "global:dbatoolsciOutput"
    }

    Context "Validate output" {
        It "Should return network interface information without errors" {
            $WarnVar | Should -BeNullOrEmpty
            $results | Should -Not -BeNullOrEmpty
        }

        It "Should have an interface name" {
            $results[0].Name | Should -Not -BeNullOrEmpty
        }

        It "Should have a node name" {
            $results[0].Node | Should -Not -BeNullOrEmpty
        }
    }
    ...
    Context "Output validation" {
        ...
        It "Should have accurate .OUTPUTS documentation" {
            ...
            $help.returnValues.returnValue.type.name | Should -Match "MSCluster_NetworkInterface"
        }
    }
}