...
Describe "$commandname Integration Tests" -Tags "IntegrationTests" {
    ...
    Context "Command excludes results for specified database" {
        ...
        It "Shouldn't get any results for $dbname" {
            $(Set-DbaDbCompression -SqlInstance $global:instance2 -Database $dbname -ExcludeDatabase $dbname -MaxRunTime 5 -PercentCompression 0).Database | Should not Match $dbname
        }
    }
