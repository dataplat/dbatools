...
Describe "$commandname Integration Tests" -Tag "IntegrationTests" {
    BeforeAll {
        ...
        $null = Set-DbaDbQueryStoreOption -SqlInstance $script:instance1 -Database $dbname -State ReadWrite
        $null = Enable-DbaTraceFlag -SqlInstance $script:instance1 -TraceFlag 7745
    }
    AfterAll {
        $null = Remove-DbaDatabase -SqlInstance $script:instance1 -Database $dbname -Confirm:$false
        $null = Disable-DbaTraceFlag -SqlInstance $script:instance1 -TraceFlag 7745
    }
    ...
    Context 'Exclude database works' {
        ...
        $results = Test-DbaDbQueryStore -SqlInstance $script:instance1 -ExcludeDatabase $dbname
        It 'Should return results' {
            $results | Should Not BeNullOrEmpty
        }
