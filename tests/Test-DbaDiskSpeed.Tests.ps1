...
Describe "$CommandName Integration Tests" -Tag "IntegrationTests" {
    Context "Command actually works" {
        It "should have info for model" {
            $results = Test-DbaDiskSpeed -SqlInstance $global:instance1
            $results.FileName -contains 'modellog.ldf' | Should -Be $true
        }
        It "returns only for master" {
            $results = Test-DbaDiskSpeed -SqlInstance $global:instance1 -Database master
        ...
        It "sample pipeline" {
            $results = @($global:instance1, $global:instance2) | Test-DbaDiskSpeed -Database master
            ...
            #(($results.SqlInstance -contains $global:instance1) -and ($results.SqlInstance -contains $global:instance2)) | Should -Be $true
        }

        It "multiple databases included" {
            $databases = @('master', 'model')
            $results = Test-DbaDiskSpeed -SqlInstance $global:instance1 -Database $databases
            $results.Count | Should -Be 4
        ...
        It "multiple databases excluded" {
            ...
            $results = Test-DbaDiskSpeed -SqlInstance $global:instance1 -ExcludeDatabase $excludedDatabases
        ...
        It "default aggregate by file" {
            $resultsWithParam = Test-DbaDiskSpeed -SqlInstance $global:instance1 -AggregateBy "File"
            $resultsWithoutParam = Test-DbaDiskSpeed -SqlInstance $global:instance1
        ...
        It "aggregate by database" {
            $results = Test-DbaDiskSpeed -SqlInstance $global:instance1 -AggregateBy "Database"
            #$databases = Get-DbaDatabase -SqlInstance $global:instance1
        ...
        It "aggregate by disk" {
            $results = Test-DbaDiskSpeed -SqlInstance $global:instance1 -AggregateBy "Disk"
            (($results -is [System.Data.DataRow]) -or ($results.count -ge 1))   | Should -Be $true
            #($results.SqlInstance -contains $global:instance1)                  | Should -Be $true
        }
        ...
        It "aggregate by file and check column names returned" {
            ...
            $results = Test-DbaDiskSpeed -SqlInstance $global:instance1 # default usage of command with no params is equivalent to AggregateBy = "File"
            ...
            if ( ($null -ne $results) ) {
                ...
                if ($results -is [System.Data.DataRow]) {
                    ...
                    Write-Message -Level Warning -Message "Unexpected results returned from $($global:instance1): $($results)"
                    $validColumns = $false
                }
                ...
                if ($null -ne $row) {
                    ...
                    if ( @(Compare-Object -ReferenceObject $expectedColumnArray -DifferenceObject $columnNamesReturned).Count -eq 0 ) {
                        Write-Message -Level Debug -Message "Columns matched on $($global:instance1)"
                        $validColumns = $true
                    } else {
                        Write-Message -Level Warning -Message "The columns specified in the expectedColumnArray variable do not match these returned columns from $($global:instance1): $($columnNamesReturned)"
                    }
                }
            }
        ...
        It -Skip "test commands on a Linux instance" {
            ...
            $linuxSecurePassword = ConvertTo-SecureString -String $global:instance2SQLPassword -AsPlainText -Force
            $linuxSqlCredential = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList $global:instance2SQLUserName, $linuxSecurePassword

            $results = Test-DbaDiskSpeed -SqlInstance $global:instance2 -SqlCredential $linuxSqlCredential -AggregateBy "Database"
            $databases = Get-DbaDatabase -SqlInstance $global:instance2 -SqlCredential $linuxSqlCredential

            $results.Database -contains 'model' | Should -Be $true
            $results.count                      | Should -Be $databases.count

            $results = Test-DbaDiskSpeed -SqlInstance $global:instance2 -SqlCredential $linuxSqlCredential -AggregateBy "Disk"

            (($results -is [System.Data.DataRow]) -or ($results.count -ge 1))   | Should -Be $true

            $resultsWithParam = Test-DbaDiskSpeed -SqlInstance $global:instance2 -SqlCredential $linuxSqlCredential -AggregateBy "File"
            $resultsWithoutParam = Test-DbaDiskSpeed -SqlInstance $global:instance2 -SqlCredential $linuxSqlCredential
        ...
        It -Skip "aggregate by file and check column names returned on a Linux instance" {
            ...
            $results = Test-DbaDiskSpeed -SqlInstance $global:instance2 -SqlCredential $linuxSqlCredential # default usage of command with no params is equivalent to AggregateBy = "File"
