...
Describe $CommandName -Tag IntegrationTests {
    Context "Command actually works" {
        ...
        It -Skip:$true "test commands on a Linux instance" {
            BeforeAll {
        ...
        # Separate test to run against a Linux-hosted SQL instance.
        # To run this test ensure you have specified the instance2 values for a Linux-hosted SQL instance in the Get-TestConfig
        It -Skip:$true "aggregate by file and check column names returned on a Linux instance" {
            BeforeAll {
                # check returned columns
                $expectedColumnArray = @(
                    "ComputerName",
        ...
        # Separate test to run against a Linux-hosted SQL instance.
        # To run this test ensure you have specified the instance2 values for a Linux-hosted SQL instance in the Get-TestConfig
        It -Skip:$true "aggregate by database and check column names returned on a Linux instance" {
            BeforeAll {
                # check returned columns
                $expectedColumnArray = @(
                    "ComputerName",
                ...
                $linuxSecurePassword = ConvertTo-SecureString -String $TestConfig.instance2SQLPassword -AsPlainText -Force
                $linuxSqlCredential = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList $TestConfig.instance2SQLUserName, $linuxSqlCredential
        ...
        # Separate test to run against a Linux-hosted SQL instance.
        # To run this test ensure you have specified the instance2 values for a Linux-hosted SQL instance in the Get-TestConfig
        It -Skip:$true "aggregate by disk and check column names returned on a Linux instance" {
            BeforeAll {
                # check returned columns
                $expectedColumnArray = @(
                    "ComputerName",
