...
Describe $CommandName -Tag IntegrationTests {
    ...
    Context "commands work as expected" {
        ...
        It "filters by table" {
            $result = Get-DbaDbDataClassification -SqlInstance $TestConfig.InstanceSingle -Database $dbName -Table $tableName
            $result | Should -Not -BeNullOrEmpty
        }

        It "filters by column" {
            $result = Get-DbaDbDataClassification -SqlInstance $TestConfig.InstanceSingle -Database $dbName -Column "EmailAddress"
            $result | Should -Not -BeNullOrEmpty
            $result.Column | Should -Be "EmailAddress"
        }

        It "returns nothing for unclassified column filter" {
            $result = Get-DbaDbDataClassification -SqlInstance $TestConfig.InstanceSingle -Database $dbName -Column "CreditCardNumber"
