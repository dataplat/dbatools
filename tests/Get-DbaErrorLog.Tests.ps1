...
Describe "Get-DbaErrorLog" {
    ...
    Context "Correctly gets error log messages" {
        BeforeAll {
            ...
            $l = Get-DbaLogin -SqlInstance $global:instance1 -Login $login
            if ($l) {
                Get-DbaProcess -SqlInstance $global:instance1 -Login $login | Stop-DbaProcess
                $l.Drop()
            }
            ...
            try {
                Connect-DbaInstance -SqlInstance $global:instance1 -SqlCredential $sqlCred -ErrorVariable whatever
            } catch {}
        }

        It "Has the correct default properties" {
            $expectedProps = 'ComputerName,InstanceName,SqlInstance,LogDate,Source,Text'.Split(',')
            $results = Get-DbaErrorLog -SqlInstance $global:instance1 -LogNumber 0
        ...
        It "Returns filtered results for [Source = $sourceFilter]" {
            $results = Get-DbaErrorLog -SqlInstance $global:instance1 -Source $sourceFilter
            $results[0].Source | Should -Be $sourceFilter
        }

        It "Returns filtered result for [LogNumber = 0] and [Source = $sourceFilter]" {
            $results = Get-DbaErrorLog -SqlInstance $global:instance1 -LogNumber 0 -Source $sourceFilter
            $results[0].Source | Should -Be $sourceFilter
        }

        It "Returns filtered results for [Text = $textFilter]" {
            $results = Get-DbaErrorLog -SqlInstance $global:instance1 -Text $textFilter
            $results[0].Text | Should -BeLike "*$textFilter*"
        }

        It "Returns filtered result for [LogNumber = 0] and [Text = $textFilter]" {
            $results = Get-DbaErrorLog -SqlInstance $global:instance1 -LogNumber 0 -Text $textFilter
            $results[0].Text | Should -BeLike "*$textFilter*"
        }

        It "Returns filtered results for [After = `$afterFilter]" {
            $after = Get-DbaErrorLog -SqlInstance $global:instance1 -LogNumber 1 | Select-Object -First 1
        ...
        It "Returns filtered result for [Before = `$beforeFilter]" {
            $before = Get-DbaErrorLog -SqlInstance $global:instance1 -LogNumber 1 | Select-Object -Last 1
