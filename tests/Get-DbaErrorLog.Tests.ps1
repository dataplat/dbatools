...
Describe $CommandName -Tag IntegrationTests {
    Context "Correctly gets error log messages" {
        BeforeAll {
            ...
            # Use script scope for variables accessed in It blocks
            $script:sourceFilter = "Logon"
            $script:textFilter = "All rights reserved"
            ...
            $script:afterFilter = $after.LogDate.AddMinutes(+1)
            $script:beforeFilter = $before.LogDate.AddMinutes(-1)
        ...
        It "Returns filtered results for [Source = <sourceFilter>]" {
            $results = Get-DbaErrorLog -SqlInstance $TestConfig.instance1 -Source $script:sourceFilter
            $results[0].Source | Should -Be $script:sourceFilter
        }

        It "Returns filtered result for [LogNumber = 0] and [Source = <sourceFilter>]" {
            $results = Get-DbaErrorLog -SqlInstance $TestConfig.instance1 -LogNumber 0 -Source $script:sourceFilter
            $results[0].Source | Should -Be $script:sourceFilter
        }

        It "Returns filtered results for [Text = <textFilter>]" {
            $results = Get-DbaErrorLog -SqlInstance $TestConfig.instance1 -Text $script:textFilter
            $results[0].Text | Should -BeLike "*$script:textFilter*"
        }

        It "Returns filtered result for [LogNumber = 0] and [Text = <textFilter>]" {
            $results = Get-DbaErrorLog -SqlInstance $TestConfig.instance1 -LogNumber 0 -Text $script:textFilter
            $results[0].Text | Should -BeLike "*$script:textFilter*"
        }

        It "Returns filtered results for [After = <afterFilter>]" {
            $results = Get-DbaErrorLog -SqlInstance $TestConfig.instance1 -After $script:afterFilter
            $results[0].LogDate | Should -BeGreaterOrEqual $script:afterFilter
        }

        It "Returns filtered results for [LogNumber = 1] and [After = <afterFilter>]" {
            $results = Get-DbaErrorLog -SqlInstance $TestConfig.instance1 -LogNumber 1 -After $script:afterFilter
            $results[0].LogDate | Should -BeGreaterOrEqual $script:afterFilter
        }

        It "Returns filtered result for [Before = <beforeFilter>]" {
            $results = Get-DbaErrorLog -SqlInstance $TestConfig.instance1 -Before $script:beforeFilter
            $results[-1].LogDate | Should -BeLessOrEqual $script:beforeFilter
        }

        It "Returns filtered result for [LogNumber = 1] and [Before = <beforeFilter>]" {
            $results = Get-DbaErrorLog -SqlInstance $TestConfig.instance1 -LogNumber 1 -Before $script:beforeFilter
            $results[-1].LogDate | Should -BeLessOrEqual $script:beforeFilter
        }
    }
}