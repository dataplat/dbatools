...
Describe "$CommandName Integration Tests" -Tags "IntegrationTests" {
    Context "Verifying query store options are copied" {
        ...
        BeforeEach {
            ...
            $db1 = New-Object Microsoft.SqlServer.Management.SMO.Database($instance1, $db1Name)
            $db1.Create()

            $db1QSOptions = Get-DbaDbQueryStoreOption -SqlInstance $instance1 -Database $db1Name
            $originalQSOptionValue = $db1QSOptions.DataFlushIntervalInSeconds
            $updatedQSOption = $db1QSOptions.DataFlushIntervalInSeconds + 1
            $updatedDB1Options = Set-DbaDbQueryStoreOption -SqlInstance $instance1 -Database $db1Name -FlushInterval $updatedQSOption -State ReadWrite

            $db2Name = "dbatoolsci_querystoretest2"
            $db2 = New-Object Microsoft.SqlServer.Management.SMO.Database($instance1, $db2Name)
            $db2.Create()

            $db3Name = "dbatoolsci_querystoretest3"
            $db3 = New-Object Microsoft.SqlServer.Management.SMO.Database($instance1, $db3Name)
            $db3.Create()

            $db4Name = "dbatoolsci_querystoretest4"
            $db4 = New-Object Microsoft.SqlServer.Management.SMO.Database($instance1, $db4Name)
            $db4.Create()
        }
        AfterEach {
        ...
        It "Copy the query store options from one db to another on the same instance" {
            $db2QSOptions = Get-DbaDbQueryStoreOption -SqlInstance $instance1 -Database $db2Name
            $db2QSOptions.DataFlushIntervalInSeconds | Should -Be $originalQSOptionValue

            $result = Copy-DbaDbQueryStoreOption -Source $instance1 -SourceDatabase $db1Name -Destination $instance1 -DestinationDatabase $db2Name
        ...
        It "Apply to all databases except db4" {
            $db3QSOptions = Get-DbaDbQueryStoreOption -SqlInstance $instance1 -Database $db3Name
            $db3QSOptions.DataFlushIntervalInSeconds | Should -Be $originalQSOptionValue

            $result = Copy-DbaDbQueryStoreOption -Source $instance1 -SourceDatabase $db1Name -Destination $instance1 -Exclude $db4Name
            ...
            $dbQSOptions = Get-DbaDbQueryStoreOption -SqlInstance $instance1 -Database $db1Name, $db2Name, $db3Name
            ($dbQSOptions | Where-Object { $_.DataFlushIntervalInSeconds -eq ($originalQSOptionValue + 1) }).Count | Should -Be 3

            $db4QSOptions = Get-DbaDbQueryStoreOption -SqlInstance $instance1 -Database $db4Name
