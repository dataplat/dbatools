...
Describe "$CommandName Integration Tests" -Tag "IntegrationTests" {
    BeforeAll {
        ...
        New-DbaDatabase -SqlInstance $script:instance3 -Name $db
        Invoke-DbaQuery -SqlInstance $script:instance3 -Database $db -Query $sql
    }

    AfterAll {
        Remove-DbaDatabase -SqlInstance $script:instance3 -Database $db -Confirm:$false
    }

    Context "Command works" {
        It "starts with the right data" {
            Invoke-DbaQuery -SqlInstance $script:instance3 -Database $db -Query "SELECT * FROM Customer WHERE FirstName = 'Delores'" | Should -Not -Be $null
            Invoke-DbaQuery -SqlInstance $script:instance3 -Database $db -Query "SELECT * FROM Customer WHERE RandomText = '6011295760226704'" | Should -Not -Be $null
        }

        It "returns the proper output" {
            $results = Invoke-DbaDbPiiScan -SqlInstance $script:instance3 -Database $db -SampleCount 500
        ...
        It "ExcludeColumn param" {
            $results = Invoke-DbaDbPiiScan -SqlInstance $script:instance3 -Database $db -ExcludeColumn @('CustomerID', 'Firstname', 'Lastname', 'FullName', 'Address', 'Zip', 'City', 'Randomtext')
        ...
        It "Table param" {
            $results = Invoke-DbaDbPiiScan -SqlInstance $script:instance3 -Database $db -Table Customer
            $results.Count | Should -Be 27
            $results.Table | Should -Not -Contain TestTable
            $results.Table | Should -Not -Contain TestTable2

            $results = Invoke-DbaDbPiiScan -SqlInstance $script:instance3 -Database $db -Table Customer, TestTable
            $results.Count | Should -Be 28
            $results.Table | Should -Not -Contain TestTable2
        }

        It "Column param" {
            $results = Invoke-DbaDbPiiScan -SqlInstance $script:instance3 -Database $db -Table Customer -Column UnknownColumn
            $results.Count | Should -Be 2
            $results."PII-Name" | Should -Contain 'IPv4 Address'
            $results."PII-Name" | Should -Contain 'IPv6 Address'

            $results = Invoke-DbaDbPiiScan -SqlInstance $script:instance3 -Database $db -Table Customer -Column UnknownColumn, Firstname
            $results.Count | Should -Be 3
            $results."PII-Name" | Should -Contain 'IPv4 Address'
            $results."PII-Name" | Should -Contain 'IPv6 Address'
            $results."PII-Name" | Should -Contain 'First name'

            $results = Invoke-DbaDbPiiScan -SqlInstance $script:instance3 -Database $db -Table Customer, TestTable -Column UnknownColumn, Firstname
        ...
        It "Country param" {
            $results = Invoke-DbaDbPiiScan -SqlInstance $script:instance3 -Database $db -Table Customer -Country Austria
            $results.Count | Should -Be 9
            (($results | Where-Object { $_.Country -eq "Austria" })."PII-Name" -eq "National ID") | Should -Be $true

            $results = Invoke-DbaDbPiiScan -SqlInstance $script:instance3 -Database $db -Table Customer -Country Austria, Slovakia
        ...
        It "CountryCode param" {
            $results = Invoke-DbaDbPiiScan -SqlInstance $script:instance3 -Database $db -Table Customer -CountryCode SK
            $results.Count | Should -Be 9
            (($results | Where-Object { $_.CountryCode -eq "SK" })."PII-Name" -eq "National ID") | Should -Be $true

            $results = Invoke-DbaDbPiiScan -SqlInstance $script:instance3 -Database $db -Table Customer -CountryCode AT, SK
            $results.Count | Should -Be 10
            (($results | Where-Object { $_.CountryCode -eq "SK" })."PII-Name" -eq "National ID") | Should -Be $true
        ...
        It "ExcludeColumn and ExcludeTable params" {
            $results = Invoke-DbaDbPiiScan -SqlInstance $script:instance3 -Database $db -ExcludeTable Customer
            $results.Count | Should -Be 2
            ($results | Where-Object { $_."PII-Name" -eq "First name" }).Count | Should -Be 2

            $results = Invoke-DbaDbPiiScan -SqlInstance $script:instance3 -Database $db -ExcludeTable Customer, TestTable
            $results.Table | Should -Be TestTable2

            $results = Invoke-DbaDbPiiScan -SqlInstance $script:instance3 -Database $db -ExcludeColumn UnknownColumn
            $results.Count | Should -Be 27
            $results.Column | Should -Not -Contain UnknownColumn

            $results = Invoke-DbaDbPiiScan -SqlInstance $script:instance3 -Database $db -ExcludeColumn UnknownColumn, FirstName
