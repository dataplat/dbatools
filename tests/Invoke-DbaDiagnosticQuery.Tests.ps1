...
Describe "$CommandName Integration Tests" -Tags "IntegrationTests" {
    BeforeAll {
		...
		write-verbose "Temporary output location for test: $script:PesterOutputPath"
        ...
        write-verbose "Created Test Databases: $database , $database2 on $script:instance2"
    }
    AfterAll {
        ...
        write-verbose "Cleaned up Test Databases: $database , $database2 on $script:instance2"

        Remove-Item $script:PesterOutputPath -recurse #clear test folder contents
        write-verbose "cleaned output location for test: $script:PesterOutputPath"

    }
    AfterEach {
        write-verbose "clearing test artifacts from $script:PesterOutputPath"
		Remove-Item "$script:PesterOutputPath\*" -Recurse #clear test folder contents
    ...
    context "verifying output when exporting queries as files instead of running" {
        ...
        It "returns pscustomobject[] containing parsed queries" {
            ...
            @($results).count | Should -BeGreaterThan 10
            $results | foreach-object { write-verbose  "foreach-object in `$results - UnderlyingSystemType $($_.GetType().UnderlyingSystemType)"  -verbose }
