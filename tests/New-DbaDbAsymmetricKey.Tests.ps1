...
Describe "$CommandName Integration Tests" -Tag "IntegrationTests" {
    ...
    Context "commands work as expected" {
        ...
        $results = Get-DbaDbAsymmetricKey -SqlInstance $global:instance2 -Name $keyname -Database master -WarningVariable warnvar
    ...
    Context "Handles pre-existing key" {
        $keyname = 'test1'
        $key = New-DbaDbAsymmetricKey -SqlInstance $global:instance2 -Name $keyname -Database master -WarningVariable warnvar 3> $null
    ...
    Context "Handles Algorithm changes" {
        $keyname = 'test2'
        $algorithm = 'Rsa4096'
        $key = New-DbaDbAsymmetricKey -SqlInstance $global:instance2 -Name $keyname -Algorithm $algorithm -WarningVariable warnvar
    ...
    Context "Non master database" {
        ...
        $key = New-DbaDbAsymmetricKey -SqlInstance $global:instance2 -Database $database -Name $keyname -Owner keyowner -Algorithm $algorithm -WarningVariable warnvar
    ...
    Context "Sets owner correctly" {
        ...
        $dbuser = 'keyowner'
        $database = 'enctest'
        $key = New-DbaDbAsymmetricKey -SqlInstance $global:instance2 -Name $keyname -Owner keyowner -Database $database -Algorithm $algorithm -WarningVariable warnvar
        $results = Get-DbaDbAsymmetricKey -SqlInstance $global:instance2 -Name $keyname -Database $database
    ...
    Context "Create new key loaded from a keyfile" {
        ...
        $dbuser = 'keyowner'
        $database = 'enctest'
        $path = "$($global:appveyorlabrepo)\keytests\keypair.snk"
        if (Test-Path -Path $path) {
            $key = New-DbaDbAsymmetricKey -SqlInstance $global:instance2 -Database $database -Name $keyname -Owner keyowner -WarningVariable warnvar -KeySourceType File -KeySource $path
            $results = Get-DbaDbAsymmetricKey -SqlInstance $global:instance2 -Name $keyname -Database $database
        } else {
    ...
    Context "Failed key creation from a missing keyfile" {
        ...
        $dbuser = 'keyowner'
        $database = 'enctest'
        $path = "$($global:appveyorlabrepo)\keytests\keypair.bad"
        $key = New-DbaDbAsymmetricKey -SqlInstance $global:instance2 -Database $database -Name $keyname -Owner keyowner -WarningVariable warnvar -KeySourceType File -KeySource $path 3> $null
        $results = Get-DbaDbAsymmetricKey -SqlInstance $global:instance2 -Name $keyname -Database $database
