...
Describe "$commandname Integration Tests" -Tags "IntegrationTests" {
    BeforeAll {
        ...
        foreach ($FrequencySubdayType in ('Time', 'Seconds', 'Minutes', 'Hours')) {
            $variables = @{SqlInstance    = $global:instance2
    ...
    Context "Should remove schedules" {
        ...
        $null = Remove-DbaAgentSchedule -SqlInstance $global:instance2 -Schedule dbatoolsci_Minutes -Confirm:$false
        ...
        $null = Get-DbaAgentSchedule -SqlInstance $global:instance2 | Where-Object { $_.name -like 'dbatools*' } | Remove-DbaAgentSchedule -Confirm:$false -Force
        $results = Get-DbaAgentSchedule -SqlInstance $global:instance2 | Where-Object { $_.name -like 'dbatools*' }
