...
Describe "Set-DbaDbOwner Integration Tests" -Tag "IntegrationTests" {
    BeforeAll {
        $svr = Connect-DbaInstance -SqlInstance $env:instance1
        $owner = "dbatoolssci_owner_$(Get-Random)"
        $ownertwo = "dbatoolssci_owner2_$(Get-Random)"
        $null = New-DbaLogin -SqlInstance $env:instance1 -Login $owner -Password ('Password1234!' | ConvertTo-SecureString -asPlainText -Force)
        $null = New-DbaLogin -SqlInstance $env:instance1 -Login $ownertwo -Password ('Password1234!' | ConvertTo-SecureString -asPlainText -Force)
        ...
        $null = New-DbaDatabase -SqlInstance $env:instance1 -Name $dbnametwo -Owner sa
    }
    AfterAll {
        $null = Remove-DbaDatabase -SqlInstance $env:instance1 -Database $dbname, $dbnametwo -Confirm:$false
        $null = Remove-DbaLogin -SqlInstance $env:instance1 -Login $owner, $ownertwo -Confirm:$false
    }
    Context "Should set the database owner" {
        It "Sets the database owner on a specific database" {
            $results = Set-DbaDbOwner -SqlInstance $env:instance1 -Database $dbName -TargetLogin $owner
    ...
    Context "Sets multiple database owners" {
        BeforeAll {
            $results = Set-DbaDbOwner -SqlInstance $env:instance1 -Database $dbName, $dbnametwo -TargetLogin $ownertwo
        }
    ...
    Context "Excludes databases" {
        BeforeAll {
            $svr.Databases[$dbName].refresh()
            $results = Set-DbaDbOwner -SqlInstance $env:instance1 -ExcludeDatabase $dbnametwo -TargetLogin $owner
        }
    ...
    Context "Enables input from Get-DbaDatabase" {
        BeforeAll {
            ...
            $db = Get-DbaDatabase -SqlInstance $env:instance1 -Database $dbnametwo
    ...
    Context "Sets database owner to sa" {
        BeforeAll {
            $results = Set-DbaDbOwner -SqlInstance $env:instance1
        }
        It "Sets the database owner on multiple databases" {
            foreach ($r in $results) {
