...
Describe $CommandName -Tag IntegrationTests {
    BeforeAll {
        $results = Get-DbaWsfcResourceGroup -ComputerName $TestConfig.InstanceHadr -WarningVariable WarnVar -OutVariable "global:dbatoolsciOutput"
    }

    Context "Validate output" {
        It "Should return resource group information without errors" {
            $WarnVar | Should -BeNullOrEmpty
            $results | Should -Not -BeNullOrEmpty
        }

        It "Should have a resource group name" {
