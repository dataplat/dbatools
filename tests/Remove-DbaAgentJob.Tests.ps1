...
Describe "$CommandName Integration Tests" -Tag "IntegrationTests" {
    Context "Command removes jobs" {
        ...
        AfterAll {
            if (Get-DbaAgentSchedule -SqlInstance $global:instance3 -Schedule dbatoolsci_daily) { Remove-DbaAgentSchedule -SqlInstance $global:instance3 -Schedule dbatoolsci_daily -Confirm:$false }
        }
        $null = Remove-DbaAgentJob -SqlInstance $global:instance3 -Job dbatoolsci_testjob -Confirm:$false
    ...
    Context "Command removes job but not schedule" {
        ...
        AfterAll {
            if (Get-DbaAgentSchedule -SqlInstance $global:instance3 -Schedule dbatoolsci_weekly) { Remove-DbaAgentSchedule -SqlInstance $global:instance3 -Schedule dbatoolsci_weekly -Confirm:$false }
        }
        $null = Remove-DbaAgentJob -SqlInstance $global:instance3 -Job dbatoolsci_testjob_schedule -KeepUnusedSchedule -Confirm:$false
    ...
    Context "Command removes job but not history and supports piping" {
        ...
        It "Should have deleted job: dbatoolsci_testjob_history" {
            $null = Get-DbaAgentJob -SqlInstance $global:instance3 -Job dbatoolsci_testjob_history | Remove-DbaAgentJob -KeepHistory -Confirm:$false
