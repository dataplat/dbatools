...
Describe $CommandName -Tag IntegrationTests -Skip {
    ...
    Context "When repairing a mirrored database" {
        It "Should repair the mirrored database" {
            $result = Repair-DbaDbMirror -SqlInstance $TestConfig.InstanceMulti1 -Database $db1 -Confirm:$false -OutVariable "global:dbatoolsciOutput"
