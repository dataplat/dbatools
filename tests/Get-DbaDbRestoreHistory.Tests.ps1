...
Describe "Get-DbaDbRestoreHistory Integration Tests" -Tag "IntegrationTests" {
    BeforeAll {
        ...
        $null = Get-DbaDatabase -SqlInstance $global:instance2 -Database $dbname1, $dbname2 | Remove-DbaDatabase -Confirm:$false
        $null = Restore-DbaDatabase -SqlInstance $global:instance2 -Path $global:appveyorlabrepo\singlerestore\singlerestore.bak -DatabaseName $dbname1 -DestinationFilePrefix $dbname1
        $null = Restore-DbaDatabase -SqlInstance $global:instance2 -Path $global:appveyorlabrepo\singlerestore\singlerestore.bak -DatabaseName $dbname2 -DestinationFilePrefix $dbname2
        $null = Restore-DbaDatabase -SqlInstance $global:instance2 -Path $global:appveyorlabrepo\singlerestore\singlerestore.bak -DatabaseName $dbname2 -DestinationFilePrefix "rsh_pre_$dbname2" -WithReplace
        $fullBackup = Backup-DbaDatabase -SqlInstance $global:instance2 -Database $dbname1 -Type Full
        $diffBackup = Backup-DbaDatabase -SqlInstance $global:instance2 -Database $dbname1 -Type Diff
        $logBackup = Backup-DbaDatabase -SqlInstance $global:instance2 -Database $dbname1 -Type Log

        $null = Restore-DbaDatabase -SqlInstance $global:instance2 -Path $diffBackup.BackupPath, $logBackup.BackupPath -DatabaseName $dbname1 -WithReplace
    }
    ...
    Context "Get last restore history for single database" {
        BeforeAll {
            $results = @(Get-DbaDbRestoreHistory -SqlInstance $global:instance2 -Database $dbname2 -Last)
        }
        ...
        It "Should return the full restore with the correct properties" {
            ...
            $results[0].From | Should -Be $global:appveyorlabrepo\singlerestore\singlerestore.bak
    ...
    Context "Get last restore history for multiple database" {
        BeforeAll {
            $results = @(Get-DbaDbRestoreHistory -SqlInstance $global:instance2 -Database $dbname1, $dbname2 -Last)
        }
    ...
    Context "Get complete restore history for multiple database" {
        BeforeAll {
            $results = @(Get-DbaDbRestoreHistory -SqlInstance $global:instance2 -Database $dbname1, $dbname2)
        }
    ...
    Context "Get restore history by restore type" {
        ...
        It "returns the correct history records for differential restore" {
            $results = Get-DbaDbRestoreHistory -SqlInstance $global:instance2 -Database $dbname1 -RestoreType Differential
        ...
        It "returns the correct history records for log restore" {
            $results = Get-DbaDbRestoreHistory -SqlInstance $global:instance2 -Database $dbname1 -RestoreType Log
            $results.Database | Should -Be $dbname1
