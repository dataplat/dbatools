...
try {
    ...
    $global:skipRepl = -not $replServer.IsDistributor
} catch {
    $global:skipRepl = $true
}
...
Describe $CommandName -Tag IntegrationTests {
    BeforeAll {
        ...
        if (-not $global:skipRepl) {
            # Disable distribution to capture output
            $global:dbatoolsciOutput = Disable-DbaReplDistributor -SqlInstance $TestConfig.InstanceSingle -Force -Confirm:$false
        }
