...
Describe $CommandName -Tag IntegrationTests {
    Context "Parameters validation" {
        ...
        It "Stops if no Database or AllDatabases" {
            { Set-DbaDbState -SqlInstance $TestConfig.InstanceSingle -EnableException } | Should -Throw -ExpectedMessage "*You must specify*"
        }
        # TODO: The output should write a normal warning, but does not.
        It -Skip "Is nice by default" {
            $null = Set-DbaDbState -SqlInstance $TestConfig.InstanceSingle -WarningAction SilentlyContinue
            $WarVar | Should -BeLike "*You must specify*"
        }
        It "Errors out when multiple 'access' params are passed with EnableException" {
            { Set-DbaDbState -SqlInstance $TestConfig.InstanceSingle -Database $db1 -SingleUser -RestrictedUser -EnableException } | Should -Throw -ExpectedMessage "*You can only specify one of: -SingleUser,-RestrictedUser,-MultiUser*"
            { Set-DbaDbState -SqlInstance $TestConfig.InstanceSingle -Database $db1 -MultiUser -RestrictedUser -EnableException } | Should -Throw -ExpectedMessage "*You can only specify one of: -SingleUser,-RestrictedUser,-MultiUser*"
        }
        # TODO: The output should write a normal warning, but does not.
        It -Skip "Errors out when multiple 'access' params are passed without EnableException" {
            $null = Set-DbaDbState -SqlInstance $TestConfig.InstanceSingle -Database $db1 -SingleUser -RestrictedUser -WarningAction SilentlyContinue
            $WarVar | Should -BeLike "*You can only specify one of: -SingleUser,-RestrictedUser,-MultiUser*"
            $null = Set-DbaDbState -SqlInstance $TestConfig.InstanceSingle -Database $db1 -MultiUser -RestrictedUser -WarningAction SilentlyContinue
            $WarVar | Should -BeLike "*You can only specify one of: -SingleUser,-RestrictedUser,-MultiUser*"
        }
        It "Errors out when multiple 'status' params are passed with EnableException" {
            { Set-DbaDbState -SqlInstance $TestConfig.InstanceSingle -Database $db1 -Offline -Online -EnableException } | Should -Throw -ExpectedMessage "*You can only specify one of: -Online,-Offline,-Emergency,-Detached*"
            { Set-DbaDbState -SqlInstance $TestConfig.InstanceSingle -Database $db1 -Emergency -Online -EnableException } | Should -Throw -ExpectedMessage "*You can only specify one of: -Online,-Offline,-Emergency,-Detached*"
        }
        # TODO: The output should write a normal warning, but does not.
        It -Skip "Errors out when multiple 'status' params are passed without Silent" {
            $null = Set-DbaDbState -SqlInstance $TestConfig.InstanceSingle -Database $db1 -Offline -Online -WarningAction SilentlyContinue
            $WarVar | Should -BeLike "*You can only specify one of: -Online,-Offline,-Emergency,-Detached*"
            $null = Set-DbaDbState -SqlInstance $TestConfig.InstanceSingle -Database $db1 -Emergency -Online -WarningAction SilentlyContinue
            $WarVar | Should -BeLike "*You can only specify one of: -Online,-Offline,-Emergency,-Detached*"
        }
        It "Errors out when multiple 'rw' params are passed with EnableException" {
            { Set-DbaDbState -SqlInstance $TestConfig.InstanceSingle -Database $db1 -ReadOnly -ReadWrite -EnableException } | Should -Throw -ExpectedMessage "*You can only specify one of: -ReadOnly,-ReadWrite*"
        }
        # TODO: The output should write a normal warning, but does not.
        It -Skip "Errors out when multiple 'rw' params are passed without EnableException" {
            { Set-DbaDbState -SqlInstance $TestConfig.InstanceSingle -Database $db1 -ReadOnly -ReadWrite *> $null } | Should -Throw -ExpectedMessage "*You can only specify one of: -ReadOnly,-ReadWrite*"
        }
    }
    Context "Operations on databases" {
        BeforeAll {
            ...
            Get-DbaDatabase -SqlInstance $TestConfig.InstanceSingle -Database $db1, $db2, $db3, $db4, $db5, $db6, $db7, $db8 | Remove-DbaDatabase
            $server.Query("CREATE DATABASE $db1")
        ...
        AfterAll {
            $null = Set-DbaDbState -Sqlinstance $TestConfig.InstanceSingle -Database $db1, $db2, $db3, $db4, $db5, $db7 -Online -ReadWrite -MultiUser -Force
            $null = Remove-DbaDatabase -SqlInstance $TestConfig.InstanceSingle -Database $db1, $db2, $db3, $db4, $db5, $db6, $db7, $db8
        }
        ...
        It "Honors the ExcludeDatabase parameter" {
            ...
            $results = Set-DbaDbState -SqlInstance $TestConfig.InstanceSingle -ExcludeDatabase $alldbs -Online -Force
        ...
        It "Sets a database as online" {
            ...
            $null = Set-DbaDbState -SqlInstance $TestConfig.InstanceSingle -Database $db1 -Emergency -Force
            $result = Set-DbaDbState -SqlInstance $TestConfig.InstanceSingle -Database $db1 -Online -Force
        ...
        It "Sets a database as offline" {
            ...
            $result = Set-DbaDbState -SqlInstance $TestConfig.InstanceSingle -Database $db2 -Offline -Force
            $result.DatabaseName | Should -Be $db2
        ...
        It "Sets a database as emergency" {
            ...
            $result = Set-DbaDbState -SqlInstance $TestConfig.InstanceSingle -Database $db3 -Emergency -Force
        ...
        It "Sets a database as single_user" {
            ...
            $result = Set-DbaDbState -SqlInstance $TestConfig.InstanceSingle -Database $db4 -SingleUser -Force
        ...
        It "Sets a database as multi_user" {
            ...
            $null = Set-DbaDbState -SqlInstance $TestConfig.InstanceSingle -Database $db6 -RestrictedUser -Force
            $result = Set-DbaDbState -SqlInstance $TestConfig.InstanceSingle -Database $db6 -MultiUser -Force
        ...
        It "Sets a database as restricted_user" {
            ...
            $result = Set-DbaDbState -SqlInstance $TestConfig.InstanceSingle -Database $db5 -RestrictedUser -Force
        ...
        It "Sets a database as read_write" {
            ...
            $null = Set-DbaDbState -SqlInstance $TestConfig.InstanceSingle -Database $db7 -ReadOnly -Force
            $result = Set-DbaDbState -SqlInstance $TestConfig.InstanceSingle -Database $db7 -ReadWrite -Force
        ...
        It "Sets a database as read_only" {
            ...
            $result = Set-DbaDbState -SqlInstance $TestConfig.InstanceSingle -Database $db8 -ReadOnly -Force
        ...
        It "Works when piped from Get-DbaDbState" {
            ...
            $results = Get-DbaDbState -SqlInstance $TestConfig.InstanceSingle -Database $db7, $db8 | Set-DbaDbState -Online -MultiUser -Force
            $results.Count | Should -Be 2
        ...
        It "Works when piped from Get-DbaDatabase" {
            ...
            $results = Get-DbaDatabase -SqlInstance $TestConfig.InstanceSingle -Database $db7, $db8 | Set-DbaDbState -Online -MultiUser -Force
            $results.Count | Should -Be 2
            $comparison = Compare-Object -ReferenceObject ($results.DatabaseName) -DifferenceObject (@($db7, $db8))
            $comparison.Count | Should -Be 0
        }
