...
Describe $CommandName -Tag IntegrationTests {
    ...
    Context "commands work as expected" {
        ...
        It "removes a classification from a column" {
            $classification = Get-DbaDbDataClassification -SqlInstance $TestConfig.InstanceSingle -Database $dbName -Column "Email"
            $classification | Should -Not -BeNullOrEmpty

            $result = $classification | Remove-DbaDbDataClassification
            $result.Status | Should -Be "Removed"

            $afterRemove = Get-DbaDbDataClassification -SqlInstance $TestConfig.InstanceSingle -Database $dbName -Column "Email"
            $afterRemove | Should -BeNullOrEmpty
        }

        It "removes all classifications when piping all results" {
            $before = Get-DbaDbDataClassification -SqlInstance $TestConfig.InstanceSingle -Database $dbName
            $before | Should -Not -BeNullOrEmpty

            $before | Remove-DbaDbDataClassification

            $after = Get-DbaDbDataClassification -SqlInstance $TestConfig.InstanceSingle -Database $dbName
