...
Describe "$CommandName Integration Tests" -Tag "IntegrationTests" {
    ...
    Context "Should get the table" {
        It "Gets the table" {
            (Get-DbaDbTable -SqlInstance $global:instance1).Name | Should Contain $tablename
        }
    ...
    Context "Should not get the table if database is excluded" {
        It "Doesn't find the table" {
            (Get-DbaDbTable -SqlInstance $global:instance1 -ExcludeDatabase $dbname).Name | Should Not Contain $tablename
        }
    }
}