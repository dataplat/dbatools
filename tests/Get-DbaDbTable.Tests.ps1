...
Describe "Get-DbaDbTable" {
    ...
    Context "Validate parameters" {
        ...
        It "Should have IncludeSystemDBs as a parameter" {
            $CommandUnderTest | Should -HaveParameter IncludeSystemDBs -Type SwitchParameter
        }
    ...
    Context "Should get the table" {
        It "Gets the table" {
            $result = Get-DbaDbTable -SqlInstance $script:instance1
            $result.Name | Should -Contain $tablename
        }
        It "Gets the table when you specify the database" {
            $result = Get-DbaDbTable -SqlInstance $script:instance1 -Database $dbname
            $result.Name | Should -Contain $tablename
        }
    }
    ...
    Context "Should not get the table if database is excluded" {
        It "Doesn't find the table" {
            $result = Get-DbaDbTable -SqlInstance $script:instance1 -ExcludeDatabase $dbname
