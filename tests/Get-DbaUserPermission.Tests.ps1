...
'@
    ...
    Context "Command do not return error when database as different collation" {
        ...
        $null = New-DbaDatabase -SqlInstance $global:instance1 -Name $dbName -Collation $dbCollation

        $results = Get-DbaUserPermission -SqlInstance $global:instance1 -Database $dbName -WarningVariable warnvar 3> $null
