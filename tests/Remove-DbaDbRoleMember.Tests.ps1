...
Describe "Remove-DbaDbRoleMember" {
    BeforeAll {
        ...
        $null = New-DbaLogin -SqlInstance $env:instance2 -Login $user1 -Password ('Password1234!' | ConvertTo-SecureString -asPlainText -Force)
        $null = New-DbaLogin -SqlInstance $env:instance2 -Login $user2 -Password ('Password1234!' | ConvertTo-SecureString -asPlainText -Force)
        $dbname = "dbatoolsci_$(Get-Random)"
        $null = New-DbaDatabase -SqlInstance $env:instance2 -Name $dbname -Owner sa
        $null = New-DbaDbUser -SqlInstance $env:instance2 -Database $dbname -Login $user1 -Username 'User1'
        $null = New-DbaDbUser -SqlInstance $env:instance2 -Database $dbname -Login $user2 -Username 'User2'
        $null = New-DbaDbUser -SqlInstance $env:instance2 -Database msdb -Login $user1 -Username 'User1' -IncludeSystem
        $null = New-DbaDbUser -SqlInstance $env:instance2 -Database msdb -Login $user2 -Username 'User2' -IncludeSystem
    ...
    AfterAll {
        ...
        $null = Remove-DbaLogin -SqlInstance $env:instance2 -Login $user1, $user2 -Confirm:$false
    }
    ...
    Context "Functionality" {
        ...
        It 'Removes Role for User' {
            $roleDB = Get-DbaDbRoleMember -SqlInstance $env:instance2 -Database $dbname -Role $role
            Remove-DbaDbRoleMember -SqlInstance $env:instance2 -Role $role -User 'User1' -Database $dbname -Confirm:$false
