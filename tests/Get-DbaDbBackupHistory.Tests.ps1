...
Describe "Get-DbaDbBackupHistory Integration Tests" -Tag "IntegrationTests" {
    BeforeAll {
        ...
        $null = Get-DbaDatabase -SqlInstance $global:instance1 -Database $dbname, $dbnameForked | Remove-DbaDatabase -Confirm:$false
        ...
        $db | Backup-DbaDatabase -Type Log -BackupDirectory $DestBackupDir
        $null = Get-DbaDatabase -SqlInstance $global:instance1 -Database master | Backup-DbaDatabase -Type Full -BackupDirectory $DestBackupDir
    ...
    Context "Get last history for single database" {
        BeforeAll {
            $results = Get-DbaDbBackupHistory -SqlInstance $global:instance1 -Database $dbname -Last
        }
    ...
    Context "Get last history for all databases" {
        BeforeAll {
            $results = Get-DbaDbBackupHistory -SqlInstance $global:instance1
        }
    ...
    Context "ExcludeDatabase is honored" {
        It "Should not report about excluded database master" {
            $results = Get-DbaDbBackupHistory -SqlInstance $global:instance1 -ExcludeDatabase 'master'
        ...
        It "Should not report about excluded database master with -Type Full" {
            $results = Get-DbaDbBackupHistory -SqlInstance $global:instance1 -ExcludeDatabase 'master' -Type Full
            ($results | Where-Object Database -match "master").Count | Should -Be 0
        }
        It "Should not report about excluded database master with -LastFull" {
            $results = Get-DbaDbBackupHistory -SqlInstance $global:instance1 -ExcludeDatabase 'master' -LastFull
            ($results | Where-Object Database -match "master").Count | Should -Be 0
        }
    }
    ...
    Context "LastFull should work with multiple databases" {
        BeforeAll {
            $results = Get-DbaDbBackupHistory -SqlInstance $global:instance1 -Database $dbname, master -lastfull
        }
    ...
    Context "Testing IncludeCopyOnly with LastFull" {
        BeforeAll {
            $results = Get-DbaDbBackupHistory -SqlInstance $global:instance1 -LastFull -Database $dbname
            $resultsCo = Get-DbaDbBackupHistory -SqlInstance $global:instance1 -LastFull -IncludeCopyOnly -Database $dbname
        }
    ...
    Context "Testing IncludeCopyOnly with Last" {
        BeforeAll {
            $resultsCo = Get-DbaDbBackupHistory -SqlInstance $global:instance1 -Last -IncludeCopyOnly -Database $dbname
        }
    ...
    Context "Testing IgnoreDiff parameter for #6914" {
        BeforeAll {
            $noIgnore = Get-DbaDbBackupHistory -SqlInstance $global:instance1 -Database $dbname
            $Ignore = Get-DbaDbBackupHistory -SqlInstance $global:instance1 -Database $dbname -IgnoreDiffBackup
        }
    ...
    Context "Testing the Since parameter" {
        It "DateTime for -Since" {
            $results = Get-DbaDbBackupHistory -SqlInstance $global:instance1 -Database $dbname -Since (Get-Date).AddMinutes(-5)
            $results.count | Should -BeGreaterThan 0
        }

        It "TimeSpan for -Since" {
            $results = Get-DbaDbBackupHistory -SqlInstance $global:instance1 -Database $dbname -Since (New-TimeSpan -Minutes -5)
            $results.count | Should -BeGreaterThan 0
        }

        It "Invalid type for -Since" {
            $results = Get-DbaDbBackupHistory -SqlInstance $global:instance1 -Database $dbname -Since "-" -WarningVariable warning 3> $null
