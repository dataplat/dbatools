...
Describe "Update-DbaMaintenanceSolution" {
    ...
    Context "Command usage" {
        ...
        It "Updates the maintenance solution" {
            $result = Update-DbaMaintenanceSolution -SqlInstance $env:instance1 -Database master
        ...
        It "Throws an exception when an invalid database is specified" {
            { Update-DbaMaintenanceSolution -SqlInstance $env:instance1 -Database 'InvalidDB' -EnableException } | Should -Throw
        }

        It "Updates only specified solutions" {
            $result = Update-DbaMaintenanceSolution -SqlInstance $env:instance1 -Database master -Solution 'IndexOptimize'
            $result | Should -Not -BeNullOrEmpty
        ...
        It "Uses a local file when specified" {
            ...
            $result = Update-DbaMaintenanceSolution -SqlInstance $env:instance1 -Database master -LocalFile $localFile
            $result | Should -Not -BeNullOrEmpty
            $result.Status | Should -Be 'Updated'
        }
    }
