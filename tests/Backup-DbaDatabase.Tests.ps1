...
Describe "$commandname Integration Tests" -Tags "IntegrationTests" {
    ...
    Context "Properly restores a database on the local drive using Path" {
        $results = Backup-DbaDatabase -SqlInstance $global:instance1 -BackupDirectory C:\temp\backups
    ...
    BeforeAll {
        ...
        Get-DbaDatabase -SqlInstance $global:instance2 -Database $DestDbRandom | Remove-DbaDatabase -Confirm:$false
    }
    AfterAll {
        Get-DbaDatabase -SqlInstance $global:instance1 -Database "dbatoolsci_singlerestore" | Remove-DbaDatabase -Confirm:$false
        Get-DbaDatabase -SqlInstance $global:instance2 -Database $DestDbRandom | Remove-DbaDatabase -Confirm:$false
    ...
    Context "No database found to backup should raise warning and null output" {
        $results = Backup-DbaDatabase -SqlInstance $global:instance1 -BackupDirectory $DestBackupDir -Database AliceDoesntDBHereAnyMore -WarningVariable warnvar 3> $null
        It "Should not return object" {
            $results | Should -Be $null
        }
    ...
    Context "Should take path and filename" {
        $results = Backup-DbaDatabase -SqlInstance $global:instance1 -BackupDirectory $DestBackupDir -Database master -BackupFileName 'PesterTest.bak'
    ...
    Context "Database parameter works when using pipes (fixes #5044)" {
        $results = Get-DbaDatabase -SqlInstance $global:instance1 | Backup-DbaDatabase -Database master -BackupFileName PesterTest.bak -BackupDirectory $DestBackupDir
        It "Should report it has backed up to the path with the correct name" {
            $results.Fullname | Should -BeLike "$DestBackupDir*PesterTest.bak"
        }
        It "Should have backed up to the path with the correct name" {
...
"@
        ...
        It "Should compress an encrypted db" {
            ...
            Invoke-Command2 -ComputerName $global:instance2 -ScriptBlock { Remove-Item -Path $args[0] } -ArgumentList $results.FullName
...
"@
    ...
    #     $key = New-DbaDbAsymmetricKey -SqlInstance $global:instance2 -Database master -Name BackupKey
    #     $encBackupResults = Backup-DbaDatabase -SqlInstance $global:instance2 -Database master -EncryptionAlgorithm AES128 -EncryptionKey BackupKey
    #     It "Should encrypt the backup" {
    #         $encBackupResults.EncryptorType | Should Be "CERTIFICATE"
    #         $encBackupResults.KeyAlgorithm | Should Be "aes_128"
    #     }
