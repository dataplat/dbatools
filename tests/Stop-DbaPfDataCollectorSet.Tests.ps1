...
Describe "$CommandName Integration Tests" -Tags "IntegrationTests" {
    BeforeAll {
        ...
        $global:set | Start-DbaPfDataCollectorSet -WarningAction SilentlyContinue
        Start-Sleep 2
    }
    AfterAll {
    ...
    Context "Verifying command works" {
        It "returns a result with the right computername and name is not null" {
            $results = $global:set | Select-Object -First 1 | Stop-DbaPfDataCollectorSet -WarningAction SilentlyContinue -WarningVariable warn
            if (-not $warn) {
