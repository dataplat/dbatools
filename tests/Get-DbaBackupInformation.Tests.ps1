...
Describe "$CommandName Integration Tests" -Tags "IntegrationTests" {
    ...
    Context "Check the export/import of backup history" {
        ...
        # Get-DbaBackupInformation -Import -Path "$DestBackupDir\history.xml" | Restore-DbaDatabase -SqlInstance $global:instance1 -DestinationFilePrefix hist -RestoredDatabaseNamePrefix hist -TrustDbBackupHistory
    ...
    Context "Test Maintenance solution options" {
        ...
        $ResultsSanLog = Get-DbaBackupInformation -SqlInstance $global:instance1 -Path $DestBackupDirOla -IgnoreLogBackup -WarningVariable warnvar -WarningAction SilentlyContinue 3> $null
