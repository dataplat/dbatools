    ...
    Context "Operations on snapshots" {
        ...
        if ($setupright) {
            It "Honors the Database parameter, restoring only snapshots of that database" {
                $result = Restore-DbaDbSnapshot -SqlInstance $script:instance2 -Database $db2 -Force
                ...
                $result = Restore-DbaDbSnapshot -SqlInstance $script:instance2 -Database $db1 -Force
                $result.Status | Should Be "Restored"
            ...
            It "Honors the Snapshot parameter" {
                $result = Restore-DbaDbSnapshot -SqlInstance $script:instance2 -Snapshot $db1_snap1 -Force
                $result.Database | Should Be $db1
                $result.Status | Should Be "Restored"
            ...
            It "Stops if multiple snapshot for the same db are passed" {
                $result = Restore-DbaDbSnapshot -SqlInstance $script:instance2 -Snapshot $db1_snap1, $db1_snap2 -Force *> $null
