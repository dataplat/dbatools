...
Describe "Restore-DbaDbSnapshot Integration Tests" -Tag "IntegrationTests" {
    BeforeAll {
        ...
        Get-DbaProcess -SqlInstance $env:instance2 | Where-Object Program -match dbatools | Stop-DbaProcess -Confirm:$false -WarningAction SilentlyContinue
        ...
        Remove-DbaDbSnapshot -SqlInstance $env:instance2 -Database $db1, $db2 -Confirm:$false
        Get-DbaDatabase -SqlInstance $env:instance2 -Database $db1, $db2 | Remove-DbaDatabase -Confirm:$false
    ...
    AfterAll {
        Remove-DbaDbSnapshot -SqlInstance $env:instance2 -Database $db1, $db2 -Confirm:$false -ErrorAction SilentlyContinue
        Remove-DbaDatabase -Confirm:$false -SqlInstance $env:instance2 -Database $db1, $db2 -ErrorAction SilentlyContinue
    }

    Context "Parameters validation" {
        It "Stops if no Database or Snapshot" {
            { Restore-DbaDbSnapshot -SqlInstance $env:instance2 -EnableException } | Should -Throw "You must specify"
        }
    ...
    Context "Operations on snapshots" {
        BeforeEach {
            $null = New-DbaDbSnapshot -SqlInstance $env:instance2 -Database $db1 -Name $db1_snap1 -ErrorAction SilentlyContinue
            $null = New-DbaDbSnapshot -SqlInstance $env:instance2 -Database $db1 -Name $db1_snap2 -ErrorAction SilentlyContinue
            $null = New-DbaDbSnapshot -SqlInstance $env:instance2 -Database $db2 -Name $db2_snap1 -ErrorAction SilentlyContinue
        }
        ...
        It "Honors the Database parameter, restoring only snapshots of that database" {
            $result = Restore-DbaDbSnapshot -SqlInstance $env:instance2 -Database $db2 -Confirm:$false -EnableException -Force
            $result.Status | Should -Be "Normal"
            $result.Name | Should -Be $db2

            $server.Query("INSERT INTO [$db1].[dbo].[Example] values ('sample2')")
            $result = Restore-DbaDbSnapshot -SqlInstance $env:instance2 -Database $db1 -Confirm:$false -Force
            $result.Name | Should -Be $db1

            # the other snapshot has been dropped
            $result = Get-DbaDbSnapshot -SqlInstance $env:instance2 -Database $db1
            $result.Count | Should -Be 1

            # the query doesn't return records inserted before the restore
            $result = Invoke-DbaQuery -SqlInstance $env:instance2 -Query "SELECT * FROM [$db1].[dbo].[Example]" -QueryTimeout 10
            $result.id | Should -Be 1
        }

        It "Honors the Snapshot parameter" {
            $result = Restore-DbaDbSnapshot -SqlInstance $env:instance2 -Snapshot $db1_snap1 -Confirm:$false -EnableException -Force
            $result.Name | Should -Be $db1
            $result.Status | Should -Be "Normal"
        ...
        It "Stops if multiple snapshot for the same db are passed" {
            $result = Restore-DbaDbSnapshot -SqlInstance $env:instance2 -Snapshot $db1_snap1, $db1_snap2 -Confirm:$false *> $null
            $result | Should -Be $null
        }

        It "has the correct default properties" {
            $result = Get-DbaDbSnapshot -SqlInstance $env:instance2 -Database $db2
