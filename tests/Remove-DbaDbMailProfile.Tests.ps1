...
Describe "$CommandName Unit Tests" -Tag 'UnitTests' {
    Context "Validate parameters" {
        ...
        [object[]]$knownParameters = 'SqlInstance', 'SqlCredential', 'Name', 'ExcludeProfile', 'InputObject', 'EnableException'
...
Describe "$CommandName Integration Tests" -Tag "IntegrationTests" {
    ...
    Context "commands work as expected" {
        ...
        It "removes a database mail profile" {
            (Get-DbaDbMailProfile -SqlInstance $server, $server2 -Name $profilename ) | Should -Not -BeNullOrEmpty
            Remove-DbaDbMailProfile -SqlInstance $server, $server2 -Name $profilename -Confirm:$false
            (Get-DbaDbMailProfile -SqlInstance $server, $server2 -Name $profilename ) | Should -BeNullOrEmpty
        }

        It "supports piping database mail profile" {
            (Get-DbaDbMailProfile -SqlInstance $server, $server2 -Name $profilename ) | Should -Not -BeNullOrEmpty
            Get-DbaDbMailProfile -SqlInstance $server, $server2 -Name $profilename | Remove-DbaDbMailProfile -Confirm:$false
            (Get-DbaDbMailProfile -SqlInstance $server, $server2 -Name $profilename ) | Should -BeNullOrEmpty
        }

        It "removes all database mail profiles but excluded" {
            (Get-DbaDbMailProfile -SqlInstance $server, $server2 -Name $profilename2 ) | Should -Not -BeNullOrEmpty
