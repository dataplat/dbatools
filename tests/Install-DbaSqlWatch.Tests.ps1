...
Describe "$CommandName Integration Tests" -Tags "IntegrationTests" {
    Context "Testing SqlWatch installer" {
        ...
        It "Installed tables" {
            $tableCount = (Get-DbaDbTable -SqlInstance $global:instance2 -Database $Database | Where-Object { $PSItem.Name -like "sqlwatch_*" }).Count
            $tableCount | Should -BeGreaterThan 0
        }
        It "Installed views" {
            $viewCount = (Get-DbaDbView -SqlInstance $global:instance2 -Database $Database | Where-Object { $PSItem.Name -like "vw_sqlwatch_*" }).Count
            $viewCount | Should -BeGreaterThan 0
        }
        It "Installed stored procedures" {
            $sprocCount = (Get-DbaDbStoredProcedure -SqlInstance $global:instance2 -Database $Database | Where-Object { $PSItem.Name -like "usp_sqlwatch_*" }).Count
