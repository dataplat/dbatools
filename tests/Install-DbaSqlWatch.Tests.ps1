...
Describe "Install-DbaSqlWatch" {
    ...
    Context "Testing SqlWatch installer" {
        ...
        It "Installs to specified database: <database>" {
            $results = Install-DbaSqlWatch -SqlInstance $env:instance2 -Database $database
        ...
        It "Installed tables" {
            $tableCount = (Get-DbaDbTable -SqlInstance $env:instance2 -Database $database | Where-Object { $_.Name -like "sqlwatch_*" }).Count
            $tableCount | Should -BeGreaterThan 0
        }

        It "Installed views" {
            $viewCount = (Get-DbaDbView -SqlInstance $env:instance2 -Database $database | Where-Object { $_.Name -like "vw_sqlwatch_*" }).Count
            $viewCount | Should -BeGreaterThan 0
        }

        It "Installed stored procedures" {
            $sprocCount = (Get-DbaDbStoredProcedure -SqlInstance $env:instance2 -Database $database | Where-Object { $_.Name -like "usp_sqlwatch_*" }).Count
