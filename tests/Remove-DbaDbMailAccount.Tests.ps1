...
Describe "$CommandName Integration Tests" -Tag "IntegrationTests" {
    ...
    BeforeEach {
        ...
        $null = New-DbaDbMailAccount -SqlInstance $server, $server2 -Account $accountname -EmailAddress admin@ad.local
        $null = New-DbaDbMailAccount -SqlInstance $server, $server2 -Account $accountname2 -EmailAddress admin@ad.local
    ...
    Context "commands work as expected" {
        ...
        It "removes a database mail account" {
            (Get-DbaDbMailAccount -SqlInstance $server, $server2 -Account $accountname ) | Should -Not -BeNullOrEmpty
            Remove-DbaDbMailAccount -SqlInstance $server, $server2 -Account $accountname -Confirm:$false
            (Get-DbaDbMailAccount -SqlInstance $server, $server2 -Account $accountname ) | Should -BeNullOrEmpty
        }

        It "supports piping database mail account" {
            (Get-DbaDbMailAccount -SqlInstance $server, $server2 -Account $accountname ) | Should -Not -BeNullOrEmpty
            Get-DbaDbMailAccount -SqlInstance $server, $server2 -Account $accountname | Remove-DbaDbMailAccount -Confirm:$false
            (Get-DbaDbMailAccount -SqlInstance $server, $server2 -Account $accountname ) | Should -BeNullOrEmpty
        }

        It "removes all database mail accounts but excluded" {
            (Get-DbaDbMailAccount -SqlInstance $server, $server2 -Account $accountname2 ) | Should -Not -BeNullOrEmpty
