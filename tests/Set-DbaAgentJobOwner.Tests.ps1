...
Describe $CommandName -Tag IntegrationTests {
    Context "Output validation" {
        BeforeAll {
            ...
            $jobName = "dbatoolsci_outputtest_jobowner_$(Get-Random)"
            $null = New-DbaAgentJob -SqlInstance $TestConfig.InstanceSingle -Job $jobName -Description "Test job for output validation"
            $result = Set-DbaAgentJobOwner -SqlInstance $TestConfig.InstanceSingle -Job $jobName
        ...
        It "Has the expected Status note property" {
            $result[0].psobject.Properties["Status"] | Should -Not -BeNullOrEmpty
