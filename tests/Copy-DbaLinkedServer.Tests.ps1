...
Describe $CommandName -Tag IntegrationTests {
    ...
    Context "Output validation" {
        BeforeAll {
            ...
            $outputResult = Copy-DbaLinkedServer @splatOutputCopy
        }
        AfterAll {
        ...
        It "Returns output of the expected type" {
            if (-not $outputResult) { Set-ItResult -Skipped -Because "copy operation returned no results (connectivity issue between instances)" }
        ...
        It "Has the correct values for key properties" {
            if (-not $outputResult) { Set-ItResult -Skipped -Because "copy operation returned no results (connectivity issue between instances)" }
            $outputResult[0].Name | Should -BeExactly "dbatoolsci_outputlinked"
