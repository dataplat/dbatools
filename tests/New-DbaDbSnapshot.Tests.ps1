...
Describe "New-DbaDbSnapshot" {
    ...
    Context "Command usage" {
        ...
        AfterAll {
            Remove-DbaDbSnapshot -SqlInstance $env:instance2 -Database $db1, $db2, $db3, $db4 -Confirm:$false
            Remove-DbaDatabase -Confirm:$false -SqlInstance $env:instance2 -Database $db1, $db2, $db3, $db4
        }

        It "Stops if no Database or AllDatabases" {
            { New-DbaDbSnapshot -SqlInstance $env:instance2 -EnableException -WarningAction SilentlyContinue } | Should -Throw "You must specify"
        }
        ...
        It "Doesn't support model, master or tempdb" {
            $result = New-DbaDbSnapshot -SqlInstance $env:instance2 -EnableException -Database model, master, tempdb -WarningAction SilentlyContinue
            $result | Should -BeNullOrEmpty
        }

        It "Skips over offline databases nicely" {
            $server.Query("ALTER DATABASE $db3 SET OFFLINE WITH ROLLBACK IMMEDIATE")
            $result = New-DbaDbSnapshot -SqlInstance $env:instance2 -EnableException -Database $db3
            $result | Should -BeNullOrEmpty
        ...
        It "Refuses to accept multiple source databases with a single name target" {
            { New-DbaDbSnapshot -SqlInstance $env:instance2 -EnableException -Database $db1, $db2 -Name "dbatools_Snapped" -WarningAction SilentlyContinue } | Should -Throw
        }

        It "Halts when path is not accessible" {
            { New-DbaDbSnapshot -SqlInstance $env:instance2 -Database $db1 -Path B:\Funnydbatoolspath -EnableException -WarningAction SilentlyContinue } | Should -Throw
        }

        It "Creates snaps for multiple dbs by default" {
            $results = New-DbaDbSnapshot -SqlInstance $env:instance2 -EnableException -Database $db1, $db2
        ...
        It "Creates snap with the correct name" {
            $result = New-DbaDbSnapshot -SqlInstance $env:instance2 -EnableException -Database $db1 -Name "dbatools_SnapMe_right"
        ...
        It "Creates snap with the correct name template" {
            $result = New-DbaDbSnapshot -SqlInstance $env:instance2 -EnableException -Database $db2 -NameSuffix "dbatools_SnapMe_{0}_funny"
            $result | Should -Not -BeNullOrEmpty
        ...
        It "has the correct default properties" {
            $result = Get-DbaDbSnapshot -SqlInstance $env:instance2 -Database $db2 | Select-Object -First 1
        ...
        It "Creates multiple snaps for db with dot in the name (see #8829)" {
            $results = New-DbaDbSnapshot -SqlInstance $env:instance2 -EnableException -Database $db4
            $results | Should -Not -BeNullOrEmpty
            foreach ($result in $results) {
