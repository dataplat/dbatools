...
Describe "Set-DbaLogin" {
    BeforeAll {
        ...
        # Create the login
        New-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random", "testlogin2_$random" -Password $password1

        New-DbaDatabase -SqlInstance $global:instance2 -Name "testdb1_$random" -Confirm:$false

        $SkipLocalTest = $true # Change to $false to run the local-only tests on a local instance
    }

    AfterAll {
        Remove-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random", "testlogin2_$random" -Confirm:$false -Force
        Remove-DbaDatabase -SqlInstance $global:instance2 -Database "testdb1_$random" -Confirm:$false
    }
    ...
    Context "Validate input" {
        It "Validates -Login and -NewName aren't the same" {
            { Set-DbaLogin -SqlInstance $global:instance2 -Login testlogin -NewName testLogin -EnableException } | Should -Throw 'Login name is the same as the value in -NewName'
        }

        It "Validates -Enable and -Disable aren't used together" {
            { Set-DbaLogin -SqlInstance $global:instance2 -Login testlogin -Enable -Disable -EnableException } | Should -Throw 'You cannot use both -Enable and -Disable together'
        }

        It "Validates -GrantLogin and -DenyLogin aren't used together" {
            { Set-DbaLogin -SqlInstance $global:instance2 -Login testlogin -GrantLogin -DenyLogin -EnableException } | Should -Throw 'You cannot use both -GrantLogin and -DenyLogin together'
        }

        It "Validates -Login is required when using -SqlInstance" {
            { Set-DbaLogin -SqlInstance $global:instance2 -EnableException } | Should -Throw 'You must specify a Login when using SqlInstance'
        }

        It "Validates -Password is a SecureString or PSCredential" {
            { Set-DbaLogin -SqlInstance $global:instance2 -Login 'testLogin' -Password 'password' -EnableException } | Should -Throw 'Password must be a PSCredential or SecureString'
        }
    }

    Context "Verify command functions" {
        It "Does test login exist" {
            $logins = Get-DbaLogin -SqlInstance $global:instance2 | Where-Object { $_.Name -eq "testlogin1_$random" } | Select-Object Name
            $logins.Name | Should -Be "testlogin1_$random"
        }

        It "Verifies -NewName doesn't already exist when renaming a login" {
            $result = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random" -NewName 'sa' -EnableException
            $result.Notes | Should -Be 'New login name already exists'
        }

        It 'Change the password from a SecureString' {
            $result = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random" -SecurePassword $password2
        ...
        It 'Changes the password from a PSCredential' {
            ...
            $result = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random" -SecurePassword $cred
            $result.PasswordChanged | Should -Be $true
        }

        It "Change the password from piped Login" {
            $login = Get-DbaLogin -Sqlinstance $global:instance2 -Login "testlogin1_$random"
        ...
        It "Disable the login" {
            $result = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random" -Disable
            $result.IsDisabled | Should -Be $true
        }

        It "Enable the login" {
            $result = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random" -Enable
            $result.IsDisabled | Should -Be $false
        }

        It "Deny access to login" {
            $result = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random" -DenyLogin
            $result.DenyLogin | Should -Be $true
        }

        It "Grant access to login" {
            $result = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random" -GrantLogin
            $result.DenyLogin | Should -Be $false
        }

        It "Enforces password policy on login" {
            $result = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random" -PasswordPolicyEnforced
            $result.PasswordPolicyEnforced | Should -Be $true
        }

        It "Catches errors when password can't be changed" {
            $result = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random" -PasswordPolicyEnforced -EnableException
            $result.PasswordPolicyEnforced | Should -Be $true

            $invalidPassword = ConvertTo-SecureString -String "password1" -AsPlainText -Force

            $result = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random" -SecurePassword $invalidPassword -WarningAction 'SilentlyContinue'
            $result | Should -Be $null

            { Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random" -SecurePassword $invalidPassword -EnableException } | Should -Throw
        }

        It "Disables enforcing password policy on login" {
            $result = Get-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random"
            $result.PasswordPolicyEnforced | Should -Be $true

            $result = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random" -PasswordPolicyEnforced:$false
            $result.PasswordPolicyEnforced | Should -Be $false
        }

        It "Add roles to login" {
            $result = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random" -AddRole serveradmin, processadmin
            $result.ServerRole | Should -Be "processadmin, serveradmin"
        }

        It "Remove roles from login" {
            $result = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random" -RemoveRole serveradmin
            $result.ServerRole | Should -Be "processadmin"
        }

        It "Results include multiple changed objects" {
            $results = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random", "testlogin2_$random" -DenyLogin
        ...
        It "DefaultDatabase" {
            $results = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random" -DefaultDatabase "testdb1_$random"
            $results.DefaultDatabase | Should -Be "testdb1_$random"
        }

        It "PasswordExpirationEnabled" {
            $result = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin2_$random" -PasswordPolicyEnforced
            $result.PasswordPolicyEnforced | Should -Be $true

            $result = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random", "testlogin2_$random" -PasswordExpirationEnabled -ErrorVariable error
            ...
            $result = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random" -PasswordPolicyEnforced -PasswordExpirationEnabled
            $result.PasswordExpirationEnabled | Should -Be $true
            $result.PasswordPolicyEnforced | Should -Be $true

            $result = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random" -PasswordExpirationEnabled:$false
        ...
        It "Ensure both password policy settings can be disabled at the same time" {
            ...
            $result = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random" -PasswordPolicyEnforced:$false -PasswordExpirationEnabled:$false
            $result.PasswordExpirationEnabled | Should -Be $false
            $result.PasswordPolicyEnforced | Should -Be $false
        }

        It "Unlock" -Skip:$SkipLocalTest {
            $results = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random" -PasswordPolicyEnforced -EnableException
            ...
            for (($i = 0); $i -le 4; $i++) {
                try {
                    Connect-DbaInstance -SqlInstance $global:instance2 -SqlCredential $invalidSqlCredential
                } catch {
            ...
            $results = Get-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random"
            $results.IsLocked | Should -Be $true

            $results = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random" -Unlock
            $results | Should -BeNullOrEmpty

            $results = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random" -Unlock -Force
            ...
            $results = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random" -Unlock -SecurePassword $password1
            $results.IsLocked | Should -Be $false
        }

        It "PasswordMustChange" {
            $changeResult = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random" -PasswordMustChange -ErrorVariable error
            ...
            $changeResult = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin2_$random" -PasswordPolicyEnforced -PasswordExpirationEnabled
            $changeResult.PasswordPolicyEnforced | Should -Be $true
            $changeResult.PasswordExpirationEnabled | Should -Be $true

            $changeResult = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random", "testlogin2_$random" -PasswordMustChange -SecurePassword $password1 -ErrorVariable error
            ...
            $changeResult = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin1_$random" -PasswordMustChange -SecurePassword $password1 -PasswordPolicyEnforced -PasswordExpirationEnabled
            $changeResult.MustChangePassword | Should -Be $true
            $changeResult.PasswordChanged | Should -Be $true
            $changeResult.PasswordPolicyEnforced | Should -Be $true
            $changeResult.PasswordExpirationEnabled | Should -Be $true

            $changeResult = Set-DbaLogin -SqlInstance $global:instance2 -Login "testlogin2_$random" -PasswordMustChange -SecurePassword $password1
            $changeResult.MustChangePassword | Should -Be $true
            $changeResult.PasswordChanged | Should -Be $true

            $result = Get-DbaLogin -SqlInstance $global:instance2 -MustChangePassword
