...
Describe "$CommandName Unit Tests" -Tag "UnitTests" {
    Context "Validate parameters" {
        ...
        [object[]]$params = (Get-ChildItem function:\Set-DbaErrorLog).Parameters.Keys
...
Describe "$CommandName Integration Tests" -Tag "IntegrationTests" {
    ...
    Context "Apply LogCount to multiple instances" {
        $results = Set-DbaErrorLog -SqlInstance $script:instance2, $script:instance1 -LogCount 8 -WarningAction SilentlyContinue -WarningVariable warn
        ...
        It "returns a warning for invalid version" {
            $warn | Should Match 'not supported' 
        }
    }
    Context "Apply LogSize to multiple instances" {
        $results = Set-DbaErrorLog -SqlInstance $script:instance2, $script:instance1 -LogSize 100 -WarningAction SilentlyContinue -WarningVariable warn2
        foreach ($result in $results) {
            It 'Returns LogSize set to 100 for each instance' {
                $result.LogSize | Should Be 100
            }
        }
