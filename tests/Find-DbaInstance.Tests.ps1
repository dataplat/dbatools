...
Describe $CommandName -Tag UnitTests {
    ...
    InModuleScope dbatools {
        ...
        # This block mocks New-Object for UdpClient/TcpClient, which only intercepted the retired
        # PS implementation. The compiled cmdlet creates real .NET sockets, so this test now
        # performs a live UDP probe of the nonexistent host "sqlhost" and fails. Skipped pending a
        # compiled-path rewrite (local SSRP responder or MSTest against the port-scan logic). The
        # browser fallback-port behavior itself IS implemented in the C# (FindDbaInstanceCommand.cs
