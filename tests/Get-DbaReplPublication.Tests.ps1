    ...
    InModuleScope dbatools {
        Context "Code Validation" {
            ...
            It "Honors the PublicationType parameter" {
                ...
                $Results = Get-DbaReplPublication -SqlInstance MockServerName -Database TestDB -PublicationType Snapshot
                $Results.PublicationType | Should Be "Snapshot"
            }

            It "Stops if validate set for PublicationType is not met" {

                { Get-DbaReplPublication -SqlInstance MockServerName -PublicationType NotAPubType } | should Throw
