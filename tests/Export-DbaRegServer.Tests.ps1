...
Describe "Export-DbaRegServer" {
    ...
    Context "Integration Tests" {
        BeforeAll {
            ...
            $newServer = Add-DbaRegServer -SqlInstance $global:instance2 -ServerName $srvName -Name $regSrvName -Description $regSrvDesc
        ...
        It "Create an xml file using FilePath" {
            ...
            $results = Export-DbaRegServer -SqlInstance $global:instance2 -FilePath $outputFileName
            $results -is [System.IO.FileInfo] | Should -Be $true
        ...
        It "Create a regsrvr file using the FilePath alias OutFile" {
            ...
            $results = Export-DbaRegServer -SqlInstance $global:instance2 -OutFile $outputFileName
            $results -is [System.IO.FileInfo] | Should -Be $true
            $results.FullName | Should -Be $outputFileName
        }
        ...
        It "Create an xml file using the FilePath alias FileName in a directory that does not yet exist" {
            ...
            $results = Export-DbaRegServer -SqlInstance $global:instance2 -FileName $outputFileName
            $results -is [System.IO.FileInfo] | Should -Be $true
            $results.FullName | Should -Be $outputFileName
        }
        ...
        It "Ensure the Overwrite param is working" {
            ...
            $invalidResults = Export-DbaRegServer -SqlInstance $global:instance2 -FilePath $outputFileName
            $invalidResults.length | Should -Be 0

            # test with -Overwrite
            $resultsOverwrite = Export-DbaRegServer -SqlInstance $global:instance2 -FilePath $outputFileName -Overwrite
        ...
        It "Test with the Group param" {
            $outputFileName = "C:\temp\dbatoolsci-regsrvr-export-$random.xml"
            $results = Export-DbaRegServer -SqlInstance $global:instance2 -FilePath $outputFileName -Group $group
            $results -is [System.IO.FileInfo] | Should -Be $true
            $results.FullName | Should -Be $outputFileName
        ...
        It "Test with the Group param and multiple group names" {
            $outputFileName = "C:\temp\dbatoolsci-regsrvr-export-$random.xml"
            $results = Export-DbaRegServer -SqlInstance $global:instance2 -FilePath $outputFileName -Group @($group, $group2)
        ...
        It "Test with the ExcludeGroup param" {
            $results = Export-DbaRegServer -SqlInstance $global:instance2 -ExcludeGroup $group2
            $results -is [System.IO.FileInfo] | Should -Be $true
