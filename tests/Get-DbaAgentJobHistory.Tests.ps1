...
# NOTE: InModuleScope "dbatools" has been removed. Binary C# cmdlets do not execute
# inside the PowerShell module scope, so InModuleScope has no effect on them and
# prevents Pester mocks from being visible to the cmdlet's InvokeScript calls.
# Mocks are now installed in the test runspace directly, which is where the C#
# cmdlet's InvokeCommand.InvokeScript(false, ...) resolves command names.
Describe $CommandName -Tag UnitTests {
    BeforeAll {
        # Mock for Connect-DbaInstance.
        # The C# ConnectInstance() method calls Connect-DbaInstance via
        # InvokeCommand.InvokeScript(false, ...) -- the mock in the test runspace
        # is visible to that call since useLocalScope = false.
        #
        # NOTE: EnumJobHistory now returns [PSCustomObject] instances, not plain
        # hashtables. The C# helper GetPSPropertyInt/String accesses properties via
        # PSObject.Properties[], which does NOT see hashtable keys. PSCustomObject
        # properties ARE visible via PSObject.Properties[].
    ...
    Context "Return values" {
        ...
        It "Figures out plain outputfiles" {
            ...
            ($Results | Where-Object { $PSItem.StepID -ne 0 -and $PSItem.JobName -eq "Job1" }).OutputFileName | Should -Match "Job1Output[12]"
            # correct output for job2
            ($Results | Where-Object { $PSItem.StepID -eq 2 -and $PSItem.JobName -eq "Job2" }).OutputFileName | Should -Match "Job2Output1"
            ($Results | Where-Object { $PSItem.StepID -eq 1 -and $PSItem.JobName -eq "Job2" }).OutputFileName | Should -Be ""
        }
    }
    ...
    Context "SQL Agent Tokens" {
        It "Handles INST" {
            ...
            $Results = @(Get-DbaAgentJobHistory -SqlInstance "SQLServerName" -WithOutputFile)
            ($Results | Where-Object { $PSItem.StepID -eq 1 -and $PSItem.JobName -eq "Job1" }).OutputFileName | Should -Be "BASEServiceName__Job1Output1"
        }
        ...
        It "Handles MACH" {
            ...
            $Results = @(Get-DbaAgentJobHistory -SqlInstance "SQLServerName" -WithOutputFile)
            ($Results | Where-Object { $PSItem.StepID -eq 1 -and $PSItem.JobName -eq "Job1" }).OutputFileName | Should -Be "BASEComputerName__Job1Output1"
        }
        ...
        It "Handles SQLDIR" {
            ...
            $Results = @(Get-DbaAgentJobHistory -SqlInstance "SQLServerName" -WithOutputFile)
            ($Results | Where-Object { $PSItem.StepID -eq 1 -and $PSItem.JobName -eq "Job1" }).OutputFileName | Should -Be "BASEInstallDataDirectory__Job1Output1"
        }
        ...
        It "Handles SQLLOGDIR" {
            ...
            $Results = @(Get-DbaAgentJobHistory -SqlInstance "SQLServerName" -WithOutputFile)
            ($Results | Where-Object { $PSItem.StepID -eq 1 -and $PSItem.JobName -eq "Job1" }).OutputFileName | Should -Be "BASEErrorLog_'_`"_]_Path__Job1Output1"
        }
        ...
        It "Handles SRVR" {
            ...
            $Results = @(Get-DbaAgentJobHistory -SqlInstance "SQLServerName" -WithOutputFile)
            ($Results | Where-Object { $PSItem.StepID -eq 1 -and $PSItem.JobName -eq "Job1" }).OutputFileName | Should -Be "BASEDomainInstanceName__Job1Output1"
        }
        ...
        It "Handles STEPID" {
            ...
            $Results = @(Get-DbaAgentJobHistory -SqlInstance "SQLServerName" -WithOutputFile)
            ($Results | Where-Object { $PSItem.StepID -eq 1 -and $PSItem.JobName -eq "Job1" }).OutputFileName | Should -Be "1__Job1Output1"
        }
        ...
        It "Handles JOBID" {
            ...
            $Results = @(Get-DbaAgentJobHistory -SqlInstance "SQLServerName" -WithOutputFile)
            ($Results | Where-Object { $PSItem.StepID -eq 1 -and $PSItem.JobName -eq "Job1" }).OutputFileName | Should -Be "0x848A71E7438BD0468F8D4FC4464F9FC5__Job1Output1"
        }
        ...
        It "Handles STRTDT" {
            ...
            $Results = @(Get-DbaAgentJobHistory -SqlInstance "SQLServerName" -WithOutputFile)
            ($Results | Where-Object { $PSItem.StepID -eq 1 -and $PSItem.JobName -eq "Job1" }).OutputFileName | Should -Be "20170926__Job1Output1"
        }
        ...
        It "Handles STRTTM" {
            ...
            $Results = @(Get-DbaAgentJobHistory -SqlInstance "SQLServerName" -WithOutputFile)
            ($Results | Where-Object { $PSItem.StepID -eq 1 -and $PSItem.JobName -eq "Job1" }).OutputFileName | Should -Be "130000__Job1Output1"
        }
        ...
        It "Handles TIME" {
            ...
            $Results = @(Get-DbaAgentJobHistory -SqlInstance "SQLServerName" -WithOutputFile)
            ($Results | Where-Object { $PSItem.StepID -eq 2 -and $PSItem.JobName -eq "Job1" }).OutputFileName | Should -Be "130001__Job1Output1"
        }
    }
