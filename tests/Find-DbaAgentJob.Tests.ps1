...
Describe $CommandName -Tag IntegrationTests {
    Context "Command finds jobs using all parameters" {
        BeforeAll {
            ...
            $null = New-DbaAgentJobStep @splatJobStep1
            ...
            $null = New-DbaAgentJobStep @splatJobStep2
            ...
            $null = New-DbaAgentJobStep @splatJobStep3
        ...
        It "Should find a specific job but not an excluded job" {
            $splatFindExclude = @{
                SqlInstance    = $TestConfig.instance2
                JobName        = "*dbatoolsci*"
                ExcludeJobName = "dbatoolsci_testjob_disabled"
            }
            $results = Find-DbaAgentJob @splatFindExclude
        ...
        It "Should work with multiple wildcard passed in (see #9572)" {
            $splatMultiWildcard = @{
                SqlInstance    = $TestConfig.instance2
                JobName        = @("*dbatoolsci*", "*dbatoolsregr*")
                ExcludeJobName = "dbatoolsci_testjob_disabled"
            }
            $results = Find-DbaAgentJob @splatMultiWildcard
            $results.Status.Count | Should -Be 2
        }
    }
}