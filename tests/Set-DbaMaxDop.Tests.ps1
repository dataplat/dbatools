...
Describe "$commandname Unit Tests" -Tag "UnitTests", Set-DbaMaxDop {
    ...
    Context "Input validation" {
        ...
        It "Should Call Stop-Function. -Database, -AllDatabases and -ExcludeDatabase are mutually exclusive." {
            Set-DbaMaxDop -SqlInstance $global:instance1 -MaxDop 12 -Database $singledb -AllDatabases -ExcludeDatabase "master" | Should Be
        }
...
Describe "$commandname Integration Tests" -Tag "IntegrationTests" {
    BeforeAll {
        Get-DbaProcess -SqlInstance $global:instance1, $global:instance2 -Program 'dbatools PowerShell module - dbatools.io' | Stop-DbaProcess -WarningAction SilentlyContinue
        $singledb = "dbatoolsci_singledb"
        $dbs = "dbatoolsci_lildb", "dbatoolsci_testMaxDop", $singledb
        $null = Get-DbaDatabase -SqlInstance $global:instance2 -Database $dbs | Remove-DbaDatabase -Confirm:$false
        foreach ($db in $dbs) {
            Invoke-DbaQuery -SqlInstance $global:instance1 -Query "CREATE DATABASE $db"
            Invoke-DbaQuery -SqlInstance $global:instance2 -Query "CREATE DATABASE $db"
        }
    }
    AfterAll {
        Get-DbaDatabase -SqlInstance $global:instance1 -Database $dbs | Remove-DbaDatabase -Confirm:$false
        Get-DbaDatabase -SqlInstance $global:instance2 -Database $dbs | Remove-DbaDatabase -Confirm:$false
    }
