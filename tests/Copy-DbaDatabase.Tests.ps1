...
Describe "$commandname Integration Tests" -Tag "IntegrationTests" {
    ...
    Context "Detach Attach" {
        It "Should be success" {
            $results = Copy-DbaDatabase -Source $global:instance2 -Destination $global:instance3 -Database $detachattachdb -DetachAttach -Reattach -Force #-WarningAction SilentlyContinue
    ...
    Context "Backup restore" {
        ...
        $results = Copy-DbaDatabase -Source $global:instance2 -Destination $global:instance3 -Database $backuprestoredb -BackupRestore -SharedPath $NetworkPath 3>$null
        ...
        It  "Should say skipped" {
            $result = Copy-DbaDatabase -Source $global:instance2 -Destination $global:instance3 -Database $backuprestoredb2 -BackupRestore -SharedPath $NetworkPath 3>$null
    ...
    Context "UseLastBackup - read backup history" {
        ...
        It "copies a database successfully using backup history" {
            ...
            $results = Copy-DbaDatabase -Source $global:instance2 -Destination $global:instance3 -Database $backuprestoredb -BackupRestore -UseLastBackup 3>$null
            $results.Name -eq $backuprestoredb
            $results.Status -eq "Successful"
        }
    ...
    Context "UseLastBackup with -Continue" {
        BeforeAll {
            ...
            $null = Get-DbaDbBackupHistory -SqlInstance $global:instance2 -Database $backuprestoredb -LastFull | Restore-DbaDatabase -SqlInstance $global:instance3 -DatabaseName $backuprestoredb -NoRecovery 3>$null
            #Run diff now
        ...
        It "continues the restore over existing database using backup history" {
            ...
            $results = Copy-DbaDatabase -Source $global:instance2 -Destination $global:instance3 -Database $backuprestoredb -BackupRestore -UseLastBackup -Continue 3>$null
            $results.Name -eq $backuprestoredb
            $results.Status -eq "Successful"
        }
    ...
    Context "Copying with renames using backup/restore" {
        ...
        It "Should warn if trying to rename and prefix" {
            $null = Copy-DbaDatabase -Source $global:instance2 -Destination $global:instance3 -Database $backuprestoredb -BackupRestore -SharedPath $NetworkPath -NewName $newname -prefix pre -WarningVariable warnvar 3> $null
    ...
    Context "Copying with renames using detachattach" {
        ...
        It "Should warn and exit if newname and >1 db specified" {
            $null = Copy-DbaDatabase -Source $global:instance2 -Destination $global:instance3 -Database $backuprestoredb, RestoreTimeClean -DetachAttach -Reattach -NewName warn -WarningVariable warnvar 3> $null
