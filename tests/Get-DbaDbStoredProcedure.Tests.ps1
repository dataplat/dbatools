...
Describe "$CommandName Integration Tests" -Tags "IntegrationTests" {
    ...
    Context "Command actually works" {
        $results = Get-DbaDbStoredProcedure -SqlInstance $global:instance2 -Database $db1Name
    ...
    Context "Exclusions work correctly" {
        It "Should contain no procs from master database" {
            $results = Get-DbaDbStoredProcedure -SqlInstance $global:instance2 -ExcludeDatabase master
            $results.Database | Should -Not -Contain 'master'
        }
        It "Should exclude system procedures" {
            $results = Get-DbaDbStoredProcedure -SqlInstance $global:instance2 -Database $db1Name -ExcludeSystemSp
    ...
    Context "Piping works" {
        It "Should allow piping from string" {
            $results = $global:instance2 | Get-DbaDbStoredProcedure -Database $db1Name
            ($results | Where-Object Name -eq $procName).Name | Should -Not -BeNullOrEmpty
        }
        It "Should allow piping from Get-DbaDatabase" {
            $results = Get-DbaDatabase -SqlInstance $global:instance2 -Database $db1Name | Get-DbaDbStoredProcedure
            ($results | Where-Object Name -eq $procName).Name | Should -Not -BeNullOrEmpty
        }
    }
    ...
    Context "Search by name and schema" {
        It "Search by name" {
            $results = $global:instance2 | Get-DbaDbStoredProcedure -Database $db1Name -Name $procName
            $results.Name | Should -Be $procName
            $results.DatabaseId | Should -Be $db1.Id
        }
        It "Search by 2 part name" {
            $results = $global:instance2 | Get-DbaDbStoredProcedure -Database $db1Name -Name "$schemaName.$procName2"
            $results.Name | Should -Be $procName2
            $results.Schema | Should -Be $schemaName
        }
        It "Search by 3 part name and omit the -Database param" {
            $results = $global:instance2 | Get-DbaDbStoredProcedure -Name "$db1Name.$schemaName.$procName2"
            $results.Name | Should -Be $procName2
            $results.Schema | Should -Be $schemaName
            $results.Database | Should -Be $db1Name
        }
        It "Search by name and schema params" {
            $results = $global:instance2 | Get-DbaDbStoredProcedure -Database $db1Name -Name $procName2 -Schema $schemaName
            $results.Name | Should -Be $procName2
            $results.Schema | Should -Be $schemaName
        }
