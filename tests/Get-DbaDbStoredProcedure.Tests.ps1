...
Describe $CommandName -Tag IntegrationTests {
    ...
    Context "Output Validation" {
        BeforeAll {
            $result = Get-DbaDbStoredProcedure -SqlInstance $TestConfig.InstanceSingle -Database $db1Name -EnableException
        }

        It "Returns the documented output type" {
            $result[0] | Should -BeOfType [Microsoft.SqlServer.Management.Smo.StoredProcedure]
        }
        ...
        It "Has DatabaseId property added by dbatools" {
            $result[0].PSObject.Properties.Name | Should -Contain 'DatabaseId'
        }
    }
}