...
Describe "$CommandName Integration Tests" -Tags "IntegrationTests" {
    ...
    Context "Copy DbMail to $global:instance3" {
        ...
        foreach ($r in $results) {
            if ($r.type -in @('Mail Configuration', 'Mail Account', 'Mail Profile')) {
                It "Should have copied $($r.type) from $global:instance2" {
                    $r.SourceServer | Should Be "$global:instance2"
                }
                It "Should have copied $($r.type) to $global:instance3" {
                    $r.DestinationServer | Should Be "$global:instance3"
                }
            }
        }
    }
    ...
    Context "Copy MailServers specifically" {
        ...
        foreach ($r in $results) {
            It "Should have $($r.status) $($r.type) from $global:instance2" {
                $r.SourceServer | Should Be "$global:instance2"
                $r.status | Should Be 'Skipped'
            }
            It "Should have $($r.status) $($r.type) to $global:instance3" {
                $r.DestinationServer | Should Be "$global:instance3"
                $r.status | Should Be 'Skipped'
            }
        }
