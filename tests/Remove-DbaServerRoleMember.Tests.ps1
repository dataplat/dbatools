...
Describe "Remove-DbaServerRoleMember" {
    BeforeAll {
        ...
        $null = New-DbaLogin -SqlInstance $env:instance2 -Login $login1 -Password ('Password1234!' | ConvertTo-SecureString -asPlainText -Force)
        $null = New-DbaLogin -SqlInstance $env:instance2 -Login $login2 -Password ('Password1234!' | ConvertTo-SecureString -asPlainText -Force)
        $null = New-DbaServerRole -SqlInstance $env:instance2 -ServerRole $customServerRole -Owner sa
        Add-DbaServerRoleMember -SqlInstance $server -ServerRole $fixedServerRoles[0] -Login $login1, $login2 -Confirm:$false
    }

    AfterAll {
        $server = Connect-DbaInstance -SqlInstance $env:instance2
        $null = Remove-DbaLogin -SqlInstance $env:instance2 -Login $login1, $login2 -Confirm:$false
    }
    ...
    Context "Functionality" {
        It 'Removes Login from Role' {
            Remove-DbaServerRoleMember -SqlInstance $env:instance2 -ServerRole $fixedServerRoles[0] -Login $login1 -Confirm:$false
        ...
        It 'Removes Login from Multiple Roles' {
            ...
            Remove-DbaServerRoleMember -SqlInstance $env:instance2 -ServerRole $serverRoles -Login $login1 -Confirm:$false
        ...
        It 'Removes Custom Server-Level Role Membership' {
            Remove-DbaServerRoleMember -SqlInstance $env:instance2 -ServerRole $customServerRole -Role $fixedServerRoles[-1] -Confirm:$false
