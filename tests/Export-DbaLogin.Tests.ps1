...
Describe $CommandName -Tag IntegrationTests {
    ...
    AfterAll {
        ...
        Remove-DbaDatabase -SqlInstance $TestConfig.instance2 -Database $dbname2 -Confirm:$false -ErrorAction SilentlyContinue
        Remove-DbaLogin -SqlInstance $TestConfig.instance2 -Login $login2 -Confirm:$false -ErrorAction SilentlyContinue
        Remove-DbaLogin -SqlInstance $TestConfig.instance2 -Login $login3 -Confirm:$false -ErrorAction SilentlyContinue

        $timenow = (Get-Date -uformat "%m%d%Y%H")
        $ExportedCredential = Get-ChildItem $DefaultExportPath, $AltExportPath -ErrorAction SilentlyContinue | Where-Object Name -match "$timenow\d{4}-login.sql|Dbatoolsci_login_CustomFile.sql"
