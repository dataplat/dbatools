...
Describe "Get-DbaService" {
    ...
    Context "Validate parameters" {
        ...
        It "Should have AdvancedProperties as a non-mandatory switch parameter" {
            $CommandUnderTest | Should -HaveParameter AdvancedProperties -Type Switch -Not -Mandatory
        }
    ...
    Context "Command actually works" {
        BeforeAll {
            ...
            $agentResults = Get-DbaService -ComputerName $script:instance2 -Type Agent
            $advancedResults = Get-DbaService -ComputerName $script:instance2 -InstanceName $instanceName -Type Agent -AdvancedProperties
        }
