...
foreach ($command in $commands) {
    ...
    Describe "Test help for $commandName" {
        ...
        # :-(
        $testhelpall += 1
        if (-not ([string]::Equals($help.relatedLinks.NavigationLink.uri, "https://docs.dbatools.io/#$commandName"))) {
            # the link should point to the correct page
            It "The link for $commandName should be https://docs.dbatools.io/#$commandName" {
                $help.relatedLinks[0].NavigationLink.uri | Should -MatchExactly "https://docs.dbatools.io/#$commandName"  -Because "The web-page should be the one for the command!"
            }
            $testhelperrors += 1
        }
