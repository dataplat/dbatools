...
Describe "$commandname Integration Tests" -Tag "IntegrationTests" {
    ...
    Context "Command copies jobs properly" {
        ...
        It "did not copy dbatoolsci_copyjob_disabled" {
            Get-DbaAgentJob -SqlInstance $global:instance3 -Job dbatoolsci_copyjob_disabled | Should -Be $null
        }

        It "disables jobs when requested" {
            (Get-DbaAgentJob -SqlInstance $global:instance2 -Job dbatoolsci_copyjob_disabled).Enabled
            ...
            $results.Status | Should -Be "Successful"
            (Get-DbaAgentJob -SqlInstance $global:instance2 -Job dbatoolsci_copyjob_disabled).Enabled | Should -Be $false
            (Get-DbaAgentJob -SqlInstance $global:instance3 -Job dbatoolsci_copyjob_disabled).Enabled | Should -Be $false
        }
    }
}