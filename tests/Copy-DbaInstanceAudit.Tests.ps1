...
Describe $CommandName -Tag IntegrationTests {
    ...
    It "Surfaces the destination-connect warning instead of throwing when the destination is unreachable" {
        $connectWarning = $null
        try {
            $null = Copy-DbaInstanceAudit -Source $TestConfig.InstanceCopy1 -Destination "localhost,9999" -Audit $auditName -WarningVariable connectWarning -WarningAction SilentlyContinue -ErrorAction SilentlyContinue
        } catch {
