...
Describe "$CommandName Integration Tests" -Tags "IntegrationTests" {
    Context "Verify basics of the Find-DbaDbUnusedIndex command" {
        BeforeAll {
            Write-Message -Level Warning -Message "Find-DbaDbUnusedIndex testing connection to $global:instance2"
            Test-DbaConnection -SqlInstance $global:instance2
            ...
            $newDB = New-DbaDatabase -SqlInstance $global:instance2 -Name $dbName
        ...
        It "Should find the 'unused' index on each test sql instance" {
            ...
            foreach ($row in $results) {
                if ($row["IndexName"] -eq $indexName) {
                    Write-Message -Level Debug -Message "$($indexName) was found on $($global:instance2) in database $($dbName)"
                    $testSQLinstance = $true
                } else {
                    Write-Message -Level Warning -Message "$($indexName) was not found on $($global:instance2) in database $($dbName)"
                }
            }
            ...
            if ( ($null -ne $results) ) {
                ...
                if ($null -ne $row) {
                    ...
                    if ( @(Compare-Object -ReferenceObject $expectedColumnArray -DifferenceObject $columnNamesReturned).Count -eq 0 ) {
                        Write-Message -Level Debug -Message "Columns matched on $($global:instance2)"
                        $testSQLinstance = $true
                    } else {
                        Write-Message -Level Warning -Message "The columns specified in the expectedColumnList variable do not match these returned columns from $($global:instance2): $($columnNamesReturned)"
                    }
                }
            }
