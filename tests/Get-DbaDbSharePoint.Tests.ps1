...
Describe "$CommandName Unit Tests" -Tag 'UnitTests' {
    Context "Validate parameters" {
        ...
        [array]$knownParameters = 'SqlInstance', 'SqlCredential', 'ConfigDatabase', 'InputObject', 'EnableException'
...
Describe "$commandname Integration Tests" -Tag "IntegrationTests" {
    BeforeAll {
        ...
        . "$PSScriptRoot\..\src\bin\smo\sqlpackage.exe" /Action:Import /tsn:$script:instance2 /tdn:Sharepoint_Config /sf:$bacpac /p:Storage=File
    }
    AfterAll {
