...
Describe "Read-DbaTraceFile" {
    BeforeAll {
        ...
        $configs = $env:instance1, $env:instance2 | Get-DbaSpConfigure -Name DefaultTraceEnabled
    ...
    Context "Verifying command output" {
        It "returns results" {
            $results = Get-DbaTrace -SqlInstance $env:instance2 -Id 1 | Read-DbaTraceFile
        ...
        It "supports where for multiple servers" {
            ...
            $warn = $null
            Get-DbaTrace -SqlInstance $env:instance2 -Id 1 | Read-DbaTraceFile -Where $where -WarningAction SilentlyContinue -WarningVariable warn > $null
    ...
    Context "Verify Parameter Use" {
        It "Should execute using parameters Database, Login, Spid" {
            $warn = $null
            $results = Get-DbaTrace -SqlInstance $env:instance2 -Id 1 | Read-DbaTraceFile -Database Master -Login sa -Spid 7 -WarningAction SilentlyContinue -WarningVariable warn
            $warn | Should -BeNullOrEmpty
        }
        It "Should execute using parameters EventClass, ObjectType, ErrorId" {
            $warn = $null
            $results = Get-DbaTrace -SqlInstance $env:instance2 -Id 1 | Read-DbaTraceFile -EventClass 4 -ObjectType 4 -ErrorId 4 -WarningAction SilentlyContinue -WarningVariable warn
            $warn | Should -BeNullOrEmpty
        }
        It "Should execute using parameters EventSequence, TextData, ApplicationName, ObjectName" {
            $warn = $null
            $results = Get-DbaTrace -SqlInstance $env:instance2 -Id 1 | Read-DbaTraceFile -EventSequence 4 -TextData "Text" -ApplicationName "Application" -ObjectName "Name" -WarningAction SilentlyContinue -WarningVariable warn
            $warn | Should -BeNullOrEmpty
        }
    }
}
