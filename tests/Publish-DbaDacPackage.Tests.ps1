...
Describe "Publish-DbaDacPackage" {
    Context "Validate parameters" {
        ...
        It "Should have PublishXml parameter" {
            $CommandUnderTest | Should -HaveParameter PublishXml -Type String
        }
        ...
        It "Should have GenerateDeploymentReport parameter" {
            $CommandUnderTest | Should -HaveParameter GenerateDeploymentReport -Type Switch
        }
        ...
        It "Should have IncludeSqlCmdVars parameter" {
            $CommandUnderTest | Should -HaveParameter IncludeSqlCmdVars -Type Switch
        }
        ...
        It "Should have DacFxPath parameter" {
            $CommandUnderTest | Should -HaveParameter DacFxPath -Type String
        }
    }
    ...
    Context "Command usage" {
        ...
        BeforeAll {
            ...
            $null = $db.Query("CREATE TABLE dbo.example (id int, PRIMARY KEY (id));
                ...
                FROM sys.objects")
            $publishprofile = New-DbaDacProfile -SqlInstance $env:instance1 -Database $dbname -Path C:\temp
        }

        AfterAll {
            Remove-DbaDatabase -SqlInstance $env:instance1, $env:instance2 -Database $dbname -Confirm:$false
        ...
        Context "Dacpac tests" {
            BeforeAll {
                ...
                $dacpac = Export-DbaDacPackage -SqlInstance $env:instance1 -Database $dbname -DacOption $extractOptions
            }
            ...
            It "Performs an xml-based deployment" {
                $results = $dacpac | Publish-DbaDacPackage -PublishXml $publishprofile.FileName -Database $dbname -SqlInstance $env:instance2 -Confirm:$false
                $results.Result | Should -BeLike '*Update complete.*'
                $ids = Invoke-DbaQuery -Database $dbname -SqlInstance $env:instance2 -Query 'SELECT id FROM dbo.example'
                $ids.id | Should -Not -BeNullOrEmpty
            }

            It "Performs an SMO-based deployment" {
                $options = New-DbaDacOption -Action Publish
                $results = $dacpac | Publish-DbaDacPackage -DacOption $options -Database $dbname -SqlInstance $env:instance2 -Confirm:$false
                $results.Result | Should -BeLike '*Update complete.*'
                $ids = Invoke-DbaQuery -Database $dbname -SqlInstance $env:instance2 -Query 'SELECT id FROM dbo.example'
                $ids.id | Should -Not -BeNullOrEmpty
            }
            ...
            It "Performs an SMO-based deployment and generates a deployment report" {
                $options = New-DbaDacOption -Action Publish
                $results = $dacpac | Publish-DbaDacPackage -DacOption $options -Database $dbname -SqlInstance $env:instance2 -GenerateDeploymentReport -Confirm:$false
                $results.Result | Should -BeLike '*Update complete.*'
            ...
            It "Performs a script generation without deployment" {
                $results = $dacpac | Publish-DbaDacPackage -Database $dbname -SqlInstance $env:instance2 -ScriptOnly -PublishXml $publishprofile.FileName -Confirm:$false
                ...
                Get-DbaDatabase -SqlInstance $env:instance2 -Database $dbname | Should -BeNullOrEmpty
            ...
            It "Performs a script generation without deployment and using an input options object" {
                ...
                $results = $dacpac | Publish-DbaDacPackage -Database $dbname -SqlInstance $env:instance2 -DacOption $opts -Confirm:$false
                $results.Result | Should -BeLike '*Reporting and scripting deployment plan (Complete)*'
                $results.DatabaseScriptPath | Should -Not -BeNullOrEmpty
                Test-Path ($results.DatabaseScriptPath) | Should -Be $true
                Get-DbaDatabase -SqlInstance $env:instance2 -Database $dbname | Should -BeNullOrEmpty
        ...
        Context "Bacpac tests" {
            BeforeAll {
                ...
                $bacpac = Export-DbaDacPackage -SqlInstance $env:instance1 -Database $dbname -DacOption $extractOptions -Type Bacpac
            }
            ...
            It "Performs an SMO-based deployment" {
                ...
                $results = $bacpac | Publish-DbaDacPackage -Type Bacpac -DacOption $options -Database $dbname -SqlInstance $env:instance2 -Confirm:$false
            ...
            It "Auto detects that a .bacpac is being used and sets the Type to Bacpac" {
                $options = New-DbaDacOption -Action Publish -Type Bacpac
                $results = $bacpac | Publish-DbaDacPackage -DacOption $options -Database $dbname -SqlInstance $env:instance2 -Confirm:$false
                $results.Result | Should -BeLike '*Updating database (Complete)*'
                $ids = Invoke-DbaQuery -Database $dbname -SqlInstance $env:instance2 -Query 'SELECT id FROM dbo.example'
                $ids.id | Should -Not -BeNullOrEmpty
            }
            ...
            It "Should throw when ScriptOnly is used" {
                { $bacpac | Publish-DbaDacPackage -Database $dbname -SqlInstance $env:instance2 -ScriptOnly -Type Bacpac -EnableException -Confirm:$false } | Should -Throw
            }
        }
    }
}
