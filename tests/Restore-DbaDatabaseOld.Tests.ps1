...
Describe "$CommandName Integration Tests" -Tags "IntegrationTests" {
    ...
    Context "Properly restores a database on the local drive using Path" {
        ...
        $results = Restore-DbaDatabaseOld -SqlInstance $script:instance1 -Path $script:appeyorlabrepo\singlerestore\singlerestore.bak
    ...
    Context "Ensuring warning is thrown if database already exists" {
        $results = Restore-DbaDatabaseOld -SqlInstance $script:instance1 -Path $script:appeyorlabrepo\singlerestore\singlerestore.bak -WarningVariable warning -WarningAction SilentlyContinue
        It "Should warn" {
    ...
    Context "Properly restores a database on the local drive using piped Get-ChildItem results" {
        $results = Get-ChildItem $script:appeyorlabrepo\singlerestore\singlerestore.bak | Restore-DbaDatabaseOld -SqlInstance $script:instance1
        It "Should Return the proper backup file location" {
            $results.BackupFile | Should Be "$script:appeyorlabrepo\singlerestore\singlerestore.bak"
        }
        It "Should return successful restore" {
    ...
    Context "Database is restored with correct renamings" {
        $results = Get-ChildItem $script:appeyorlabrepo\singlerestore\singlerestore.bak | Restore-DbaDatabaseOld -SqlInstance $script:instance1 -DestinationFilePrefix prefix
        ...
        $results = Get-ChildItem $script:appeyorlabrepo\singlerestore\singlerestore.bak | Restore-DbaDatabaseOld -SqlInstance $script:instance1 -DestinationFileSuffix suffix -WithReplace
        ...
        $results = Get-ChildItem $script:appeyorlabrepo\singlerestore\singlerestore.bak | Restore-DbaDatabaseOld -SqlInstance $script:instance1 -DestinationFileSuffix suffix -DestinationFilePrefix prefix -WithReplace
	...
	Context "Replace databasename in Restored File" {
        $results = Get-ChildItem $script:appeyorlabrepo\singlerestore\singlerestore.bak | Restore-DbaDatabaseOld -SqlInstance $script:instance1 -DatabaseName Pestering -replaceDbNameInFile -WithReplace
    ...
    Context "Folder restore options" {
        $results = Get-ChildItem $script:appeyorlabrepo\singlerestore\singlerestore.bak | Restore-DbaDatabaseOld -SqlInstance $script:instance1 -DestinationDataDirectory $DataFolder
		...
		$results = Get-ChildItem $script:appeyorlabrepo\singlerestore\singlerestore.bak | Restore-DbaDatabaseOld -SqlInstance $script:instance1 -DestinationDataDirectory $DataFolder -DestinationLogDirectory $LogFolder -WithReplace
    ...
    Context "Putting all restore file modification options together" {
        $results = Get-ChildItem $script:appeyorlabrepo\singlerestore\singlerestore.bak | Restore-DbaDatabaseOld -SqlInstance $script:instance1 -DestinationDataDirectory $DataFolder -DestinationLogDirectory $LogFolder -DestinationFileSuffix Suffix -DestinationFilePrefix prefix
    ...
    Context "Properly restores an instance using ola-style backups" {
        $results = Get-ChildItem $script:appeyorlabrepo\sql2008-backups | Restore-DbaDatabaseOld -SqlInstance $script:instance1
    ...
    Context "RestoreTime setup checks" {
        $results = Restore-DbaDatabaseOld -SqlInstance $script:instance1 -path $script:appeyorlabrepo\RestoreTimeClean
    ...
    Context "RestoreTime point in time" {
        $results = Restore-DbaDatabaseOld -SqlInstance $script:instance1 -path $script:appeyorlabrepo\RestoreTimeClean -RestoreTime (get-date "2017-06-01 13:22:44")
        $sqlResults = Invoke-Sqlcmd2 -ServerInstance $script:instance1 -Query "select convert(datetime,convert(varchar(20),max(dt),120)) as maxdt, convert(datetime,convert(varchar(20),min(dt),120)) as mindt from RestoreTimeClean.dbo.steps"
    ...
    Context "RestoreTime point in time and continue" {
        $results = Restore-DbaDatabaseOld -SqlInstance $script:instance1 -path $script:appeyorlabrepo\RestoreTimeClean -RestoreTime (get-date "2017-06-01 13:22:44") -StandbyDirectory c:\temp
        $sqlResults = Invoke-Sqlcmd2 -ServerInstance $script:instance1 -Query "select convert(datetime,convert(varchar(20),max(dt),120)) as maxdt, convert(datetime,convert(varchar(20),min(dt),120)) as mindt from RestoreTimeClean.dbo.steps"
        It "Should have restored 4 files" {
            $results.count | Should be 4
        }
        ...
        $results2 = Restore-DbaDatabaseOld -SqlInstance $script:instance1 -path $script:appeyorlabrepo\RestoreTimeClean -Continue
    ...
    Context "Check Get-DbaBackupHistory pipes into Restore-DbaDatabaseOld" {
        $history = Get-DbaBackupHistory -SqlInstance $script:instance1 -Database RestoreTimeClean -Last
        $results = $history | Restore-DbaDatabaseOld -SqlInstance $script:instance1 -WithReplace -TrustDbBackupHistory
    ...
    Context "Restores a db with log and file files missing extensions" {
        $results = Restore-DbaDatabaseOld -SqlInstance $script:instance1 -path $script:appeyorlabrepo\sql2008-backups\Noextension.bak -ErrorVariable Errvar -WarningVariable WarnVar
    ...
    Context "Setup for Recovery Tests" {
        ...
        $results = Restore-DbaDatabaseOld -SqlInstance $script:instance1 -Path $script:appeyorlabrepo\singlerestore\singlerestore.bak -NoRecovery -DatabaseName $DatabaseName -DestinationFilePrefix $DatabaseName -WithReplace
        It "Should have restored everything successfully" {
            ($results.RestoreComplete -contains $false) | Should be $False
        }  
    ...
    Context "Test recovery via parameter" {
        $DatabaseName = 'rectest'
        $results = Restore-DbaDatabaseOld -SqlInstance $script:instance1 -Recover -DatabaseName $DatabaseName 
        It "Should have restored everything successfully" {
            ($results.RestoreComplete -contains $false) | Should be $False
        }  
        $check = Get-DbaDatabase -SqlInstance $script:instance1 -Database $DatabaseName
    ...
    Context "Test recovery via pipeline" {
        $DatabaseName = 'rectest'
        $results = Get-DbaDatabase -SqlInstance $script:instance1 -Database $DatabaseName | Restore-DbaDatabaseOld -SqlInstance $script:instance1 -Recover 
        It "Should have restored everything successfully" {
            ($results.RestoreComplete -contains $false) | Should be $False
        }  
        $check = Get-DbaDatabase -SqlInstance $script:instance1 -Database $DatabaseName
    ...
    Context "Checking we cope with a port number (#244)" {
        $DatabaseName = 'rectest'
        $results = Restore-DbaDatabaseOld -SqlInstance $script:instance1_detailed -Path $script:appeyorlabrepo\singlerestore\singlerestore.bak  -DatabaseName $DatabaseName -DestinationFilePrefix $DatabaseName -WithReplace
        It "Should have restored everything successfully" {
            ($results.RestoreComplete -contains $false) | Should be $False
        }        
    }
