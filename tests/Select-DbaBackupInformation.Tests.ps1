...
Describe "$commandname Unit Tests" -Tag 'UnitTests' {
	InModuleScope dbatools {
		Context "Empty TLog Backup Issues" {
            ...
            $Header | %{$_ | Add-Member -Name 'Type' -Type noteProperty -Value $_.BackupTypeDescription}
            $Header | %{$_ | Add-Member -Name 'Database' -Type noteProperty -Value $_.DatabaseName}
		...
		Context "General Diff Restore" {
            ...
            $Header | %{$_ | Add-Member -Name 'Database' -Type noteProperty -Value $_.DatabaseName}
			Mock Read-DbaBackupHeader { $Header }
			$Output = Select-DbaBackupInformation -SqlInstance 'TestSQL' -Files "c:\dummy.txt"
