...
Describe $CommandName -Tag IntegrationTests {
    # NOTE ON COVERAGE: removing a publication needs a configured replication publisher/publication,
    ...
    # on this destructive (drop publication) command; the guard returns before any gated action.
    Context "Guarding the input parameters" {
        It "Warns with the input guard and returns nothing when neither SqlInstance nor InputObject is supplied" {
            ...
            $result = @(Remove-DbaReplPublication @splatNoInput)
