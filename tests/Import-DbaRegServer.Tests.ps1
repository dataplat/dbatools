...
Describe "Import-DbaRegServer" {
    ...
    Context "Integration Tests" {
        BeforeAll {
            ...
            $newGroup = Add-DbaRegServerGroup -SqlInstance $env:instance2 -Name $group
            $newServer = Add-DbaRegServer -SqlInstance $env:instance2 -ServerName $srvName -Name $regSrvName -Description $regSrvDesc -Group $newGroup.Name
            ...
            $newGroup2 = Add-DbaRegServerGroup -SqlInstance $env:instance2 -Name $group2
            $newServer2 = Add-DbaRegServer -SqlInstance $env:instance2 -ServerName $srvName2 -Name $regSrvName2 -Description $regSrvDesc2

            $regSrvName3 = "dbatoolsci-server3"
            $srvName3 = "dbatoolsci-server3"
            $regSrvDesc3 = "dbatoolsci-server3desc"

            $newServer3 = Add-DbaRegServer -SqlInstance $env:instance2 -ServerName $srvName3 -Name $regSrvName3 -Description $regSrvDesc3
        }

        BeforeEach {
            Get-DbaRegServer -SqlInstance $env:instance2 | Where-Object Name -match dbatoolsci | Remove-DbaRegServer -Confirm:$false
            Get-DbaRegServerGroup -SqlInstance $env:instance2 | Where-Object Name -match dbatoolsci | Remove-DbaRegServerGroup -Confirm:$false
        }
        ...
        It "imports group objects" {
            $results = $newServer.Parent | Import-DbaRegServer -SqlInstance $env:instance2
        ...
        It "imports registered server objects" {
            $results2 = $newServer2 | Import-DbaRegServer -SqlInstance $env:instance2
        ...
        It "imports a file from Export-DbaRegServer" {
            ...
            $results4 = Import-DbaRegServer -SqlInstance $env:instance2 -Path $results3
        ...
        It "imports from a random object so long as it has ServerName" {
            ...
            $results = $object | Import-DbaRegServer -SqlInstance $env:instance2
        ...
        It "does not import object if ServerName does not exist" {
            ...
            $results = $object | Import-DbaRegServer -SqlInstance $env:instance2 -WarningAction SilentlyContinue -WarningVariable warn
